...
Desde o Pyhton 3.5, anotações precisam estar em conformidade com a PEP 484.
...
No Python 3, `map` e `filter` devolvem geradores—uma forma de iterador—então sua substituta direta é agora uma expressão geradora (no Pyhton 2, essas funções devolviam listas, então sua alternativa mais próxima era a compreensão de lista).
...
As restrições sintáticas tendem a tornar ilegíveis ou intratáveis as `lambdas` não-triviais. Se uma `lambda` é difícil de ler, aconselho fortemente seguir o comselho de Fredrik Lundh sobre refatoração.
...
A abordagem funcional para a criação de funções com  estado interno é atráves do uso de clausuras (_closures_). Clausuras e decoradores são o assunto do <<closures_and_decorators>>.

Vamos agora esplorar a poderosa sintaxe oferecida pelo Python para declarar parâmetros de funções, e para passar argumentos para elas.
...
<4> Obtém a latitute de dentro de `metro_areas[0]` .
...
O <<closures_and_decorators>> mergulha nos decoradores de função—um tipo especial de função de ordem superior—e no mescanismo de clausura (_closure_) que os faz funcionar.
...
Neese artigo, o Python é mencionado nominalmente na seguinte passagem:
