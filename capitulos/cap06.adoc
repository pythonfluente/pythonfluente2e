...
O((("object references", "further reading on"))) https://fpy.li/2j[capítulo "Modelo de Dados"] de _A Referência da Linguagem Python_ inicia com uma explicação bastante clara sobre identidades e valores de objetos.
...
Você pode encontrar mais informações sobre o coletor de lixo geracional do CPython em the https://fpy.li/3y[gc — Interface para o coletor de lixo¶], que começa com a frase "Este módulo fornece uma interface para o opcional garbage collector". O adjetivo "opcional" usado aqui pode ser surpreendente, mas o https://fpy.li/2j[capítulo "Modelo de Dados"] também afirma:
