...
Agora, no <<ex_bus1_console>> interativo, vamos criar um objeto +bus+ (+bus1+) e dois clones—uma cópia rasa (+bus2+) e uma cópia profunda (+bus3+)—para ver o que acontece quando `bus1` deixa um passageiro.
