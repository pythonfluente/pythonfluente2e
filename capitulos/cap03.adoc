...
As seções <<consequences_dict_internals>> e <<consequences_set_sec>>
resumem o que você precisa saber sobre isso para usar bem as estruturas efetadas.
...
Se você já está acostumada com as listcomps, as dictcomps são um próximo passo natural.
...
Isso funciona quando todas as chaves são strings e únicas, para todos os argumentos (porque argumentos nomeados duplicados são proibidos):
...
A((("pattern matching", "with mappings", secondary-sortas="mappings", id="PMmap03")))((("mappings", "pattern matching with",
id="Mpattern03")))((("match/case statement")))((("dictionaries and sets", "pattern matching with mappings", id="DASmap03")))
instrução `match/case` suporta sujeitos que sejam objetos mapeamento.
Padrões para mapeamentos se parecem com literais `dict`,
mas podem casar com instâncias de qualquer subclasse real ou virtual de `collections.abc.Mapping`.footnote:[Uma subclasse virtual é
qualquer classe registrada com uma chamada ao método `.register()` de uma ABC,
como explicado na <<virtual_subclass_sec>>
 Um tipo implementado através da API Python/C também é aceitável, se um bit de marcação específico estiver acionado. Veja
https://docs.python.org/pt-br/3.10/c-api/typeobj.html#Py_TPFLAGS_MAPPING[`Py_TPFLAGS_MAPPING`] (EN).]
...
que frequentemente precisamos ler de APIs JSON ou bancos de dados com schemas semi-estruturados, como o MongoDB, o EdgeDB, ou o PostgreSQL.
...
mapeada para uma sequência. Devolve os itens da sequência, como uma nova `list`.
<2> Casa com qualquer mapeamento na forma `'type': 'book', 'api' :1`, e uma chave `'author'`
mapeada para qualquer objeto. Devolve aquele objeto dentro de uma `list`.
...
mapeada para um único objeto. Devolve o objeto dentro de uma `list`.
...
Na <<mappings_flexible_sec>>, vamos estudar o `defaultdict` e outros mapeamentos onde buscas com chaves via `+__getitem__+`
(isto é, `d[chave]`) funcionam porque itens ausentes são criados na hora.
No contexto do pattern matching, um match é bem sucedido apenas se o sujeito já possui as chaves necessárias no início do bloco `match`.
...
"standard API of mapping types", id="Mapi03")))((("collections.abc module", "Mapping and MutableMapping ABCs")))((("MutableMapping ABC")))
...
Assim, todas elas compartilham a mesma limitação, as((("keys", "hashability"))) chaves precisam ser _hashable_
...
https://docs.python.org/pt-br/3/glossary.html#term-hashable[_Glossário_ de Python]:

[quote]
____
Um objeto é hashable se tem um código de hash que nunca muda durante seu ciclo de vida
(precisa ter um método __hash__()) e pode ser comparado com outros objetos (precisa ter um método __eq__()).
Objetos hashable que são comparados como iguais devem ter o mesmo código de hash.footnote:[O verbete para "hashable" no
https://docs.python.org/pt-br/3/glossary.html#term-hashable[_Glossário_ de Python]
...
Então pode ser confuso se referir ao código de hash como um valor. Nesse livro usarei apenas _código de hash_.]
____
...
API básica para mapeamentos é muito variada.
A <<mapping_methods_tbl>> mostra os métodos implementados por `dict` e por duas variantes populares:
...
assume que `m` é um mapeamento. Caso contrário, `update()` reverte para uma iteração sobre `m`,
...
ainda persistem e que cito aqui—apenas reordenando os itens conforme sua relevância no uso diário:
...
Monitorar a ordem de inserção era uma preocupação secundária.
...
https://docs.python.org/pt-br/3/library/collections.html#collections.ChainMap["Objetos ChainMap"],
...
https://docs.python.org/pt-br/3/library/collections.html#collections.Counter[documentação].
...
que acabam nos obrigando a sobrepor métodos que poderíamos apenas herdar de `UserDict`
sem maiores problemas.footnote:[O problema exato de se criar subclasses de `dict` e de outros tipos embutidos
é tratado na <<subclass_builtin_woes>>.]
...
de Raymond Hettinger. Para experimentar com a `TransformDict`,
...
mas nenhuma mudança pode ser feita através desse último. Veja uma breve demonstração no <<ex_MappingProxyType>>.
...
Os((("dictionaries and sets", "dictionary views", id="DASviews03"))) métodos de instância de `dict`
`.keys()`, `.values()`, e `.items()` devolvem instâncias de classes chamadas
...
como explicado na <<slots_section>>.]
...
quando `+__init__+` retorna, tenha os mesmos nomes de atributos que a primeira instância daquela classe a ser criada.
O `+__dict__+` de cada instância então pode manter apenas seus próprios valores de atributos como uma simples array de ponteiros.
...
Para saber mais, por favor leio o texto https://fpy.li/hashint["Internals of sets and dicts"] (EN).
...
tipos `set` e `frozenset` são ambos implementados com um tabela de hash. Isso tem os seguintes efeitos:
...
* Conjuntos usam mais memória se comparados aos simples ponteiros de um array para seus elementos—que é uma estrutura mais compacta,
mas menos eficientes para buscas quando seu tamanho cresce além de uns poucos elementos.
* A ordem dos elementos depende da ordem de inserção, mas não de forma útil ou confiável.
Se dois elementos são diferentes mas tem o mesmo código hash, sua posição depende de qual elemento foi inserido primeiro.
* Acrescentar elementos a um conjunto muda a ordem dos elementos existentes.
Isso ocorre porque o algoritmo se torna menos eficiente se a tabela de hash estiver com mais de dois terços de ocupação,
então Python pode ter que mover e redimensionar a tabela conforme ela cresce.
...
Isso encerra nossa visão geral dos recursos dos conjuntos. Como prometido na <<dictionary_views_sec>>,
...
contenha valores não-hashable gera um `TypeError: unhashable type 'T'`, sendo `T` o tipo do valor incorreto.
...
muito úteis para checagem de tipo durante a execução.
...
a seção
https://docs.python.org/pt-br/3/library/collections.html["collections—Tipos de dados de contêineres"]
...
O https://fpy.li/3-19[PyPy] foi o primeiro interpretador Python a implementar a proposta de Raymond Hettinger de dicts compactos,
...
https://fpy.li/3-26[_Objects/dictobject.c_] (EN) e no documento de design em https://fpy.li/3-27[_Objects/dictnotes.txt_] (EN).
...
inspirada por um exemplo do capítulo 6 do excelente http://gopl.io[_The Go Programming Language_ (A Linguagem de Programação Go)] (EN),
