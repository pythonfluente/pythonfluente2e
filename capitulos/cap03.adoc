...
* A seção <<modern_dict_syntax_sec>> fala da sintaxe aperfeiçoada de desempacotamento e de diferentes maneiras de fundir mapeamentos—incluindo os operadores `|` e `|=`, suportados pelos `dicts` desde o Python 3.9.
...
Essa sintaxe também pode ser usada para fundir mapas, mas isso pode ser feito de outras formas. Siga comigo.

==== Fundindo mapeamentos com |

O Python 3.9((("mappings", "merging")))((("&#x01C0; (pipe) operator")))((("pipe (&#x01C0;) operator")))((("&#x01C0;&#x3D; (pipe equals) operator")))((("pipe equals (&#x01C0;&#x3D;) operator"))) suporta o uso de `|` e `|=` para fundir mapeamentos.
...
https://fpy.li/pep584[PEP 584—Add Union Operators To dict (_Acrescentar Operadores de União a dict_)] (EN) inclui um bom resumo das outras formas de fundir mapeamentos.
====
...
Tipos núméricos((("numeric types", "hashability of"))) e os tipos planos imutáveis `str` e `bytes` são todos hashable.
...
Em outras plavras, o resultado final desta linha...
...
[WARNING]
====
O `pickle` do Pyhton é fácil de usar nos caso mais simples, mas tem vários inconvenientes.
...
* Conjuntos usam mais memória se comparados aos simples ponteiros de uma array para seus elementos—que é uma estrutura mais compacta, mas também muito mais lenta para buscas se seu tamanho cresce além de uns poucos elementos.
...
Isso encerra nossa visão geral dos resursos dos conjuntos. Como prometido na seção <<dictionary_views_sec>>, vamos agora ver como dois dos tipos de views de dicionários se comportam de forma muito similar a um `frozenset`.((("", startref="Soper03")))((("", startref="DASset03-ops")))
...
Deixe a eficiente implemtação do Pyhton em C trabalhar para você!
...
https://fpy.li/3-29["Set Practice: learning from Python's set types" (_A Prática dos Conjuntos: aprendendo com os tipos conjunto do Pyhton_)] (EN),
