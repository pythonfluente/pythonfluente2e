...
____
Gerenciadores de contexto podem acabar sendo quase tão importantes quanto a própria sub-rotina. Só arranhamos sua superfície. [...] Basic tem uma instrução `with`, há instruções `with` em várias linguagens. Mas elas não fazem a mesma coisa, todas fazem algo muito raso, economizam consultas a atributos com o operador ponto (`.`), elas não configuram e defazem ambientes. Não pense que é a mesma coisa só porque o nome é igual. A instrução `with` é muito mais que isso.footnote:[Palestra de abertura da PyCon US 2013: https://fpy.li/18-1["What Makes Python Awesome" ("_O que torna o Python incrível_")]; a parte sobre `with` começa em 23:00 e termina em 26:15.] (EN)
____
...
Já estudamos casamento de padrões em capítulos anteriores, mas aqui veremos como a gramática de uma linguagem de programação pode ser expressa como padrões de sequências.
...
=== Gerenciadores de Contexto e os Blocos with
...
O código no bloco `finally` normalmente libera um recurso crítico ou restaura um estado anterior que havia sido temporiamente modificado.
...
No topo do `with`, o Python chama o método `+__enter__+` do objeto gerenciador de contexto. Quando o bloco `with` encerra ou termina por qualquer razão, o Pyhton chama
...
<2> Imprime uma `str`, depois o valor da variável alvo `what`.
...
<6> Se tudo correu bem, o Pyhton chama `+__exit__+` com `None, None, None`; se ocorreu uma exceção, os três argumentos recebem dados da exceção, como descrito a seguir, logo após esse exemplo.
<7> Restaura o método original em `sys.stdout.write`.
<8> Se a exceção não é `None` e seu tipo é `ZeroDivisionError`, imprime uma mensagem...
...
Uma melhoria na sintaxe foi permitir gerenciadores de contexto agrupados entre parânteses, assim:
...
Pode ser que você esteja prestes a escrever algo que já existe, ou talvez exista uma classe ou algum chamável que tornará seu trabalho mais fácil.
...
`closing`:: Uma função para criar gerenciadores de contexto a partir de objetos que fornecam um método `close()` mas não implementam a interface `+__enter__/__exit__+`.
...
.mirror_gen_exc.py: gerenciador de contexto baseado em um gerador implementando tratamento de erro—com o mesmo comportamento extreno de <<looking_glass_ex>>
====
[source, py]
----
...
Lembre-se que o método `+__exit__+` diz ao interpretador que ele tratou a exceção ao devolver um valor _verdadeiro_; nessse caso, o interpretador suprime a exceção.

Por outro lado, se `+__exit__+` não devolver explicitamene um valor, o interpretador recebe o habitual `None`, e propaga a exceção.
...
<2> Atribuir com `[]` sobreescreve ou insere novos itens, mas sempre no primeiro mapeamento, `inner_env` nesse exemplo.
...
O corpo de `evaluate` é composto por uma única instrução `match` com uma expressão `exp` como tópico.
...
* Introduzir regras especiais de avaliação, como `quote` e `lambda`—que não analisam nenhuma de suas sub-expressões
...
Ação&#x3a;::
    Analisa `test`:
    * Se verdadeira, analisa `consequence` e devolve seu valor.
    * Caso contrário, analisa `alternative` e devolve seu valor.
...
Ação&#x3a;::
    Cria e devolve uma nova instância de `Procedure` com os nomes de parâmetros, a lista de expressões como o corpo e o ambiente atual.
...
um objeto chamável mantendo nomes de parâmetros, um corpo de função e uma referência ao ambiente no qual a função foi definida.
...
Ação&#x3a;::
    Analisa a expressão e coloca o valor resultante em `env`, usando `name` como chave.
...
Podemos usar aquela simples forma `define` para criar variáveis ou para vincular nomes a funções anônimas, usando `(lambda …)` como o `value_exp`.

O Scheme padrão oferece um atalho para definir funções nomeada. Essa é a segunda forma de `define:
...
O método `Environment.change` atravessa os ambientes encadeados de local para global, e atualiza a primeira ocorrência de `name` com o novo valor.
Se não estivéssemos implementando a palavra reservada `'set!'`, poderíamos usar o `ChainMap` do Python como tipo de `Environment` por toda parte neste interpretador.
...
declarar `nonlocal x` permite a `x = 10` atualizar um variável `x` anteriormente definida fora do escopo local.
...
Ação&#x3a;::
    * Analisar `func_exp` para obter uma `proc` da função.
    * Analisar cada item em `args` para criar uma lista de valores dos argumentos.
...
mas agora podemos mergulhar mais fundo e ver como uma clasura é implementada em _lis.py_:
...
<3> Chamada por `proc(*values)` na última linha da clásula `case [func_exp, *args]`.
...
Agora podemos passar para o terceiro e último sujeito desde capítulo:
os lugares incomuns onde a cláusula `else` pode aparecer no Python.((("", startref="WMEBlispy18")))((("", startref="PMlispy18")))

=== Faça isso, então aquilo: os blocos else além do if

Isso((("with, match, and else blocks", "else clause", id="WMEBelse18")))((("else blocks", id="else18"))) não é nenhum segredo, mas é um recurso subestimado da linguagem:
...
`for`:: O bloco `else` vai ser executado apenas se e quando o loop `for` rodar até o fim (isto é, não rodará se o `for` for interromido com um `break`).
...
Nós então estudamos o elegante interpretador Scheme de Peter Norvig, o _lis.py_, escrito em Pyhton idiomático e refatorado para usar `match/case` em `evaluate`—a função central de qualquer interpretador.
...
Encorajo vocês a irem lá e olharem para a ponta desse iceberb, e daí cavarem mais fundo.
...
As implementações((("Soapbox sidebars", "proper tail calls (PTC)", id="SStail18")))((("proper tail calls (PTC)", id="PTC18")))((("tail call optimization (TCO)", id="tco18"))) padrão de Scheme são obrigadas a oferecer _chamadas de cauda apropriadas_ (PTC, sigla em inglês para _proper tail calls_), para tornar a iteração por recursão uma alternativa práitca aos loops `while` das linguagens imperativas.
...
A chamada para `factorial` na última linha não é uma chamada de cauda, pois o valor de `return` não é o somente resultado de uma chamada de função:
...
// [role="pagebreak-before less_space"]
Eu provavelmente irei seguir o conselho de Norvig quando acrescentar mais funcionalidades ao
...
o exemplo é paralelo à implementacão com `if/elif/…` (<<ex_norvigs_eval>>),
as cláusulas `case` demonstram mais recursos de _pattern matching_
e código é mais conciso.((("", startref="WMEBsoap18")))
