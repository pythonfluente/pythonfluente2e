...
____
Gerenciadores de contexto podem acabar sendo quase tão importantes quanto a própria sub-rotina. Só arranhamos sua superfície. [...] Basic tem uma instrução `with`, há instruções `with` em várias linguagens. Mas elas não fazem a mesma coisa, todas fazem algo muito raso, economizam consultas a atributos com o operador ponto (`.`), elas não configuram e desfazem ambientes. Não pense que é a mesma coisa só porque o nome é igual. A instrução `with` é muito mais que isso.footnote:[Palestra de abertura da PyCon US 2013: https://fpy.li/18-1["What Makes Python Awesome" ("_O que torna o Python incrível_")]; a parte sobre `with` começa em 23:00 e termina em 26:15.] (EN)
____
...
um objeto invocável mantendo nomes de parâmetros, um corpo de função e uma referência ao ambiente no qual a função foi definida.
