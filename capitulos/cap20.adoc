...
não apenas com threads mas também com processos—úteis para tarefas de processamento intensivo.
...
Os scripts baixam imagens de _fluentpython.com_, que usa uma CDN
...
A diferença de performance entre os scripts concorrentes não é significativa, mas ambos são mais de cinco vezes mais rápidos que o script sequencial—e isto apenas para a pequena tarefa de baixar 20 arquivos, cada um com uns poucos kilobytes.
...
[WARNING]
====
Ao testar clientes HTTP concorrentes usando servidores web públicos, você pode inadvertidamentes lançar um ataque de negação de serviço (DoS, _Denial of Service attack_), ou se tornar suspeito de estar tentando um ataque.
...
. Independente dos elementos de concorrência que você use—threads ou corrotinas—haverá um ganho enorme de performance sobre código sequencial em operações de E/S de rede, se o script for escrito corretamente.
. Para clientes HTTP que podem controlar quantas requisições eles fazem, não há diferenças significativas de performance entre threads e corrotinas.footnote:[Para servidores que podem ser acessados por muitos clientes, há uma diferença: as corrotinas escalam melhor, pois usam menos memória que as threads, e também reduzem o custo das mudanças de contexto, que mencionei na seção <<thread_non_solution_sec>>.]
...
O <<flags_threadpool_ex>> mostra a forma mais fácil de implementar os downloadas de forma concorrente, usando o método `ThreadPoolExecutor.map`.
...
a classe `Future` no Tornado, e `Promisse` no Javascript moderno.
...
Ao rodar _proc_pool.py_, podemos observar não apenas a ordem descendente dos resultados, mas também que o programa parece emperrar após mostrar o resultao para 9999999999999999.
...
----
====
<1> Essa função simplemente imprime quaisquer argumentos que receba, precedidos pelo momento da execução no formato `[HH:MM:SS]`.
...
<3> Dispara um `HTTPStetusError` se o código de status do HTTP não está em `range(200, 300)`.
...
O <<flags2_dowload_many_seq>> lista a versão sequencial da função `download_many`. O código é simples, mas vale a pena estudar para compará-lo com as versões concorrentes que veremos a seguir. Se concentre em como ele informa o progresso, trata erros e conta os downloads.

[[flags2_dowload_many_seq]]
...
<13> Trata exceções em potencial; o resto dessa função é idêntica à função `download_many` no <<flags2_dowload_many_seq>>), exceto pela observação a seguir.
<14> Para dar contexto à mensagem de erro, recupera o código de país do `to_do_map`, usando o `future` atual como chave. Isso não era necessário na versão sequencial, pois estávamos iterando sobre a lista de códigos de país, então sábiamos qual era o `cc` atual; aqui estamos iterando sobre _futures_.
...
Essas tarefas são melhor desempenhadas por programadore de sistemas, que tem o conhecimento, a inclinação e o tempo para fazê-las direito—ou assim esperamos.
...
Como o Elixir, o Go é uma linguagem moderna com ideias novas. Mas, em alguns aspectos, é uma linguagem conservadora, comparada ao Elixir. O Go não tem macros, e sua sintaxe é mais simples que a do Python. O Go não suporta herança ou sobrecarga de operadores, e oferece menos oportunidades para metaprogramação que o Python. Essas limitações são consideradas benéficas. Elas levam a comportamentos e performance mais previsíveis. Isso é uma grande vantagem em ambientes de missão crítica altamente concorrentes, onde o Go pretende substituir o C++, o Java e o Python.

Enquanto o Elixir e o Go são competidores diretos no espaço da alta concorrência, seus projetos e filosofias atraem públicos diferentes. Ambos tem boas chances de prosperar, Mas na história das linguagens de programação, as linguagens mais conservadoras tendem a atrair mais programadores.
