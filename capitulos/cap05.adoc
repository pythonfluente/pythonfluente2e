...
Vamos trata de anotações de tipo nas assinaturas de função no <<ch_type_hints_def>> e de anotações mais avançadas no <<ch_more_types>>.
...
Os atributos de classe `a` e `b` são((("descriptors"))) descritores (_descriptors_)—um recurso avançado tratado no <<ch_descriptors>>.
...
Vamos tratar dos tipos genéricos no <<ch_type_hints_def>>. Por ora, observe que o  <<club_ex>> e o <<club_generic_ex>> estão ambos corretos, e que o verificador de tipagem Mypy não reclama de nenhuma das duas definições de classe.
