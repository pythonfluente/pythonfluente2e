...
++++
<blockquote>
<p>Classes de dados são como crianças. São um bom ponto de partida mas,
para participarem como um objeto adulto, precisam assumir alguma responsabilidade.</p>
<p data-type="attribution">Martin Fowler and Kent Beck <span data-type="footnote">em <em>Refactoring</em>,
primeira edição, Capítulo 3, seção "Bad Smells in Code, Data Class"
<em>(Mau cheiro no código, classe de dados)</em>, página 87 (Addison-Wesley).</span></p>
</blockquote>
++++
...
`typing.NamedTuple`:: Uma alternativa que requer dicas de tipo nos campos—desde Python 3.5,
...
potencialmente, mais complexidade—desde Python 3.7.

Após falar sobre essas fábricas de classes, vamos discutir o motivo de _classe de dados_
ser também o nome um((("code smells"))) _code smell_:
...
parecer apenas outra fábrica de classes de dados.
...
aparece nessa segunda edição do _Python Fluente_.
...
E aquele código repetitivo não nos dá sequer os recursos básicos
...
os métodos `+__init__+`, `+__repr__+`, e `+__eq__+` necessários, além alguns outros recursos úteis.
...
Aqui está uma classe `Coordinate` criada com uma `namedtuple`—uma função fábrica
que cria uma subclasse de `tuple` com o nome e os campos especificados:
...
pass:[<a href="#ch_class_metaprog" data-type="xref" data-xrefstyle="chap-num-title">#ch_class_metaprog</a>].]
para personalizar a criação da classe do usuário. Veja isso:
...
contendo as dicas de tipo para os campos.
...
Apenas `typing.NamedTuple` e `dataclass` suportam a sintaxe normal de declaração de `class`,
...
Apesar da sintaxe de declaração de classe ser mais legível, ela é fixa no código.
...
Na verdade, muitas funções da biblioteca padrão, que antes devolviam tuplas simples agora devolvem,
tuplas nomeadas, sem afetar de forma alguma o código do usuário.
...
uma tupla nomeada para manter informações sobre uma cidade.
...
Desde Python 3.8, ele devolve um `dict` simples—o que não causa qualquer problema,
...
definir a função e então atribuí-la a um atributo de classe. O <<ranked_card_ex>> mostra como isso é feito:
...
além daqueles que `collections.namedtuple` também gera—e aquele herdados de `tuple`.
A única diferença é a presença do atributo de classe `+__annotations__+`—que
Python ignora completamente durante a execução do programa.
...
"basics of", id="typehint05"))) de tipo—também chamadas anotações de tipo—são formas de
declarar o tipo esperado dos argumentos, dos valores devolvidos, das variáveis e dos atributos de funções.

A primeira coisa que você precisa saber sobre dicas de tipo é que elas não são impostas de forma alguma
...
Vamos trata de anotações de tipo nas assinaturas de função no <<ch_type_hints_def>>
...
de programas em Python durante a execução. Veja o <<no_runtime_check_ex>>.
...
A seção https://fpy.li/5-6["Acceptable type hints" (_Dicas de tipo aceitáveis)], na PEP 484,
explica o que são tipo aceitáveis.
...
Vimos((("variable annotations", "meaning of", id="VAmean05"))), no tópico <<no_runtime_effect_sec>>,
...
não existe no Python. Obrigado, Guido!]
...
Os((("@dataclass", "default settings"))) defaults são, de fato,
...
classe ou qualquer outro invocável, que será chamado com zero argumentos,
...
Aqui `<factory>` é um caminho mais curto para dizer que algum invocável vai produzir
o valor default para `guests` (no nosso caso, a fábrica é a classe `list`).
O ponto é o seguinte: para fornecer um `handle` mas não um `guests`,
...
`InitVar` vai evitar que `@dataclass` trate `database` como um campo regular.
...
O segundo detalhe é que cheiros nem sempre indicam um problema. Alguns métodos longos são bons.
...
para dar início a um novo projeto ou módulo. Com o tempo, a classe deve ganhar seus próprios métodos,
...
e nesses casos é aceitável deixar o esboço pronto para uso.
...
o último item deve ser um tupla de dois elementos, com duas instâncias de `float`.
...
pass:[<a href="https://fpy.li/pep634" class="orm:hideurl">PEP 634—Structural Pattern Matching:
Specification (<em>(Pattern Matching Estrutural: Especificação)</em></a>]:
...
Se a classe não de um daqueles onze tipos embutidos "abençoados",
então essas variáveis parecidas com  argumentos representam padrões
a serem testados com atributos de uma instância daquela classe.
...
é a máquina do tempo de Guido, um aparelho que se diz que ele possui por causa da
frequência irritante com que pedidos de usuários por novos recursos
recebem como resposta "Implementei isso noite passada mesmo..."
____
...
Primeiro, quando você usa `@dataclass`, dicas de tipo não são opcionais.
Pelos últimos sete anos, desde a
https://fpy.li/pep484[PEP 484—Type Hints (_Dicas de Tipo_)] (EN),
nos prometeram que elas sempre seriam opcionais.
...
pode querer usar a https://fpy.li/5-24[`attrs`] no lugar do `@dataclass`.
...
subitamente estamos de volta os bons velhos tempos quando declarações
