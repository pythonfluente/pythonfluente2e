...
A programação orientada a objetos((("interfaces", "role in object-oriented programming"))) diz respeito integralmente a interfaces.
A melhor abordagem para entender um tipo em Python é conhecer os métodos que aquele tipo oferece - sua interface -, como discutimos na <<types_defined_by_ops_sec>> do (<<type_hints_in_def_ch>>).

Dependendo ((("interfaces", "ways of defining and using"))) da linguagem de programação, temos uma ou mais maneiras de definir e usar interfaces. Desde o Python 3.8, temos quatro maneiras. Elas estão retratadas no [.keep-together]#_Mapa de Sistemas de Tipagem_# (<<type_systems_described>>). Podemos resumí-las assim:

Duck typing (_tipagem pato_)::
    A((("duck typing"))) abordagem default do Python para tipagem desde o início. Estamos estudando duck typing desde <<data_model>>.
Goose typing (_tipagem ganso_)::
    A((("goose typing", "definition of term"))) abordagem suportada pelas classes base abstratas (ABCs, _sigla em inglês para Abstract Base Classes_) desde o Python 2.6, 
    que depende de verificações dos objetos como as ABCs durante a execução. A _tipagem ganso_ é o assunto principal desse capítulo.
Tipagem estática::
    A((("static typing"))) abordagem tradicional das linguagens de tipos estáticos como C e Java; suportada desde o Python 3.5 pelo módulo `typing`, e aplicada por verificadores de tipo externos compatíveis com a https://fpy.li/pep484[PEP 484—Type Hints].
    Este não é o tema desse capítulo. A maior parte do <<type_hints_in_def_ch>> e do <<more_types_ch>> mais à frente são sobre tipagem estática.
Duck typing estática::
    Uma((("static duck typing"))) abordagem popularizada pela linguagem Go; suportada por subclasses de `typing.Protocol` - lançada no Python 3.8 e também aplicada com o suporte de verificadores de tipo externos. Tratamos desse tema pela primeira vez em <<protocols_in_fn>> (<<type_hints_in_def_ch>>).


=== O Mapa de Tipagem

As((("interfaces", "typing map")))((("typing map"))) quatro abordagens retratadas na <<type_systems_described>> são complementares: elas tem diferentes prós e contras. Não faz sentido descartar qualquer uma delas.
...
image::images/flpy_1301.png[Quatro abordagens para verificaçào de tipo]

Cada uma dessas quatro abordagens dependem de interfaces para funcionarem, mas a tipagem estática pode ser implementada de forma pobre - usando apenas tipos concretos em vez de abstrações de interfaces como protocolos e classes base abstratas.
Este capítulo é sobre duck typing, goose typing (_tipagem ganso_), e duck typing estática - disciplinas de tipagem que circulam em torno de interfaces.
...
Esperamos que uma sequência também ofereça `len()`, através da implementação de `__len__`.
...
Também é assim que protocolos sào entendidos em Smalltalk, o primeiro ambiente de programação orientado a objetos a usar esse termo.

Exceto em páginas sobre programação de redes, a maioria dos usos da palavra "protocolo" na documentação do Pyhton se refere a essas interfaces informais.
...
Além de protocolos estáticos, o Pyhton também oferece outra forma de definir uma interface explícita no código: uma classe base abstrata (ABC).
...
[[python_digs_seq_sec]]
==== O Python Entende as Sequências

A filosofia do Modelo de Dados do Pyhton é cooperar o máximo possível com os protocolos dinâmicos essenciais.
...
Só estou usando a figura para descrever o quê uma `Sequence` completa deve oferecer.
...
Da mesma forma que o Python é esperto o suficiente para iterar sobre instâncias de `Vowels`, ele também consegue fazer o operador `in` funcionar memso quando o método `__contains__` não existe: ele faz uma busca sequencial para verificar se o item está presente.
...
O protocolo iterável em Pyhton representa uma forma extrema de duck typing:
o interpretador tenta dois metodos diferentes para iterar sobre objetos.
...
Se você estiver curiosa(o), olhe o código-fonte destes métodos em https://fpy.li/13-3[_Lib/_collections_abc.py_].
...
"Monkey patching"((("protocols", "implementing at runtime", id="Prun13")))((("monkey-patching", id="monkey13"))) é a ação de modificar dinamicamente um módulo, uma classe ou uma função durante a execuçào do código, para acrescentar funcionalidade ou corrigir bugs.
...
modificar uma classe ou módulo durante a execuçào, sem tocar no código finte.
...
quando você escreve código que aceita uma sequência de ítens para processar internamente como uma `list`, não imponha um argumento `list` através de checagem de tipo.
...
Mais tarde os dois contribuiram com revisões técnicas minuciosas.
...
Isso importa? Depende do contexto! Para o propósito de decidir como cozinhar uma ave depois de caçá-la, por exemplo, características observáveis específicas (mas nem todas - a plumagem, por exemplo, é de mínima importância nesse contexto), especialmente textura e sabor (a boa e velha fenética), podem ser muito mais relevantes que a cladística. Mas para outros problemas, tal como a suscebilidade a diferentes patógenos (se você estiver tentando criar aves aquáticas em cativeiro, ou preservá-las na natureza), a proximidade do DNA por ser muito mais crucial.
...
Alex tambem aponta que herdar de uma ABC é mais que implementar os métodos necessários:
é também uma declaraçào de intenções clara da parte do desenvolvedor.
...
Se usadas com classes concretas, verificações de tipo limitam o polimorfismo - um recurso essencial da programaçào orientada a objetos.
...
pass:[<span class="keep-together"><code>"Can't instantiate</code></span><code>&nbsp;abstract class FrenchDeck2 with abstract methods __delitem__, insert"</code>] ("_Impossível instanciar a classe abstrata FrenchDeck2 com os métodos abstratos __delitem__, _insert_").
...
Os métodos concretos em cada ABC de `collections.abc` ABC são implementados nos termos da interface pública da classe, então funcionam sem qualquer conhecimento da estrutura interna das instâncias.
...
Por exemplo, `__contains__` funciona executando uma busca sequencial na sequência, mas se sua a sua sequência concreta mantém seus itens e ordenados, você pode escrever um  `__contains__` que execute uma busca binária usando a função https://fpy.li/13-13[`bisect`] da biblioteca padrão.
...
Para usar bem as ABCs, você precisa saber o quê está dispinível. Vamos então revisar as ABCs de `collections` a seguir.((("", startref="GTsub13")))((("", startref="ABCsub13")))((("", startref="IASabcs13")))
...
O <<collections_uml>> é((("UML class diagrams", "ABCs in collections.abc"))) um diagrama de classe resumido (sem os nomes dos atributos) das 17 ABCs definidas em `collections.abc`.
...
Para deixar claro aos usuário do `ADAM` o quê se espera de um componente de "escolha aleatória não repetida", vamos definir uma ABC.
...
mas mostra que podemos confiar em `.pick()` e `.load(…)` para inspecionar o que está dentro de `Tombola`, puxando e devolvendo os ítens - sem saber como eles são efetivamente armazenados.
...
////
PROD: I improvised "callouts" with number dingbats in the image and text after it. Sorry. If you have a better solution, please do it. Thanks!
////
...
[quote]
____
Quando abstractmethod() é aplicado em combinação com outros descritores de método, ele deve ser aplicado como o decorador mais interno...footnote:[O verbete https://docs.python.org/pt-br/dev/library/abc.html#abc.abstractmethod[`@abc.abstractmethod`] na https://docs.python.org/pt-br/dev/library/abc.html[documentação do módulo `abc`].]
____

Em outras palavrs, nenhum outro decorador pode aparecer entre `@abstractmethod` e o comando `def`.
====
...
====
[role="pagebreak-before less_space"]
<1> O inicializador aceita qualquer iterável: o argumento é usado para construir uma lista.
...
(isto é, nós não meramente atribuimos `self._balls = iterable`, apelidando o argumento).
...
E mesmo se nós sempre recebecemos listas no argumento `iterable`,
...
O método `register` normalmente é invocado como uma funçào comum (veja <<register_usage>>),
...
Criar uma subclasse de uma  ABC ou se registrar com uma ABC são ambas maneiras explícitas de fazer nossas classes passarem verificações com `issubclass` e `isinstance` (que também se apoia em `issubclass`).
...
No Python 3.7, Ivan Levkivskyi and Inada Naoki reescreveram em C a maior parte da lógica do módulo `abc`, para melhorar a performance.
Veja https://fpy.li/13-27[Python issue #31333].
A implementação atual de `ABCMeta.__subclasscheck__` simplemente chama `_abc_subclasscheck`.
...
Todas as implementações de `__subclasshook__` que eu vi no código-fonte do Python estão em ABCs como `Sized`, que declara apenas um método especial, e elas simplemente verificam a presença do nome daquele método especial.
...
Por isso a classe https://fpy.li/13-29[`abc.Sequence`] não implementa [.keep-together]#`__subclasshook__`#.
...
Vamos encerrar esse capítulo ilustrando os protocolos estáticos com dois exemplos simples, e uma discusssão sobre as ABCs numéricas e protocolos.
...
Como é impossível nomear todos os tipos que implementam um protocolo (suportando as operações requeridas), a duck typing não podia ser descrita por dicas de tipo antes do Pyhton 3.8.
...
O parâmetro `self` normalmente não é anotado - se pressume que seu tipo seja a classe.
...
Mas incluir dicas de tipo em `complex.__float__` no _typeshed_ não resolveria esse problema, porque o interpretador Python em geral ignora dicas de tipo - e, de qualquer forma, não tem acesso aos arquivos stub do _typeshed_.
...
E isso não vai mudar tão cedo, porque este tipo de verificação de tipo durante a execução traria um custo de performance inaceitável.footnote:[Agradeço a Ivan Levkivskyi,
...
==== Suportanto um Protocolo Estático
...
Quando isso acontecer, a importaçào será redundante mas inofensiva.
...
Martin Fowler escreveu um post definindo a https://fpy.li/13-33[_interface papel_]footnote:["papel" aqui é usado no sentido de incorporação de um personagem (NT)], uma ideia útil de ter em mente ao desenvolver protocolos.

Além disso, algumas vezes vemos um protocolo definido próximo a uma função que o usa - ou seja, definido em "código do cliente" em vez de ser definido em uma bibliotecaa separada.
Isso torna mais fácil criar novos tipos para chamar aquela função, algo bom para a extensibilidade e para testes com simulações ou protótipos.
...
==== As ABCs em numbers e os Protocolos Numéricos
...
apesar de, no _typeshed_, apenas a classe embutida `complex` ter o método `__complex__`.footnote:[Isso é uma mentira bem intencionada da parte do typeshed: a partir do Python 3.9, o tipo embutido `complex` na verdade não tem amis um método `__complex__`.]
...
Finalizamos aquela seção vendo como o método especial `__subclasshook__` permite às ABCs suportarem a tipagem estrutural, pelo reconhecimento de classes não-relacionadas, mas que fornecem os métodos que preenchem os requisistos da interface definida na ABC.
...
Vimos como o decorador `@runtime_checkable` tambem aproveita o `__subclasshook__` para suportar tipagem estrutural durante a execução - mesmo que o melhor uso dos protocolos estáticos seja com verificadores de tipo estáticos, 
...
O capítulo terminou com <<numbers_abc_proto_sec>>, que conta a triste história do estado lastismável da torre numérica e das poucas insuficiências existentes na  alternativa proposta:
os protocolos numéricos estáticos tal como `SupportsFloat` e outros, adicionados ao módulo `typing` no Python 3.8.
...
Dito isso, o Pyhton ganhou sua enorme popularidade enquanto suportava apenas duck typing.
...
inclui alguns argumentos sobre a razão da torrre numérica ser inadequada para verificação estática de tipo.
...
e a https://fpy.li/pep560[PEP 560—Core support for typing module and generic types], que permitiram que coleções embutidas e da biblioteca padrão aceitem dicas de tipo genéricas "de fábrica" no Pyhton 3.9, graças à https://fpy.li/pep585[PEP 585—Type Hinting Generics In Standard Collections].
...
Se há uma coisa que todos aprendemos com o Python 3, é que progresso incremental é mais seguro que lançamentos estrondosos. Estou contente que não tivemos que esperar pelo Python 4 - se é que existirá - para tornar o Pyhton mais atrativo par grandes empresas, onde os benefícios da tipagem estática superam a complexidade adicional.
...
mAo contrário de Ruby e Javascript, o Python não permite modificações de tipos embutidos durante a execução. Eu na verdade considero isso uma vantagem, pois dá a certeza que um objeto `str` vai sempre ter os mesmos métodos.
...
Esse é o tema principal de _Python Fluente_: ressaltar os idiomas fundamentais da linguagem, para que o seu código seja conciso, efetivo e legível - para um Pyhonista fluente.((("", startref="ABCsoap13")))((("", startref="Psoap13")))((("", startref="Isoap13")))
