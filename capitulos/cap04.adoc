...
Python 3 introduziu uma forte distinção entre strings de texto humano e sequências de bytes puros.
...
mas que também inclui um utilitário de comando de linha, `chardetect`.
...
https://docs.python.org/pt-br/3/library/codecs.html#encodings-and-unicode[documentação sobre codecs] (EN)
de Python:
...
https://docs.python.org/pt-br/3/using/cmdline.html#envvar-PYTHONIOENCODING[`PYTHONIOENCODING`]—agora
essa variável é ignorada, a menos que
https://docs.python.org/pt-br/3/using/cmdline.html#envvar-PYTHONLEGACYWINDOWSSTDIO[`PYTHONLEGACYWINDOWSSTDIO`]
...
Entretanto, a
https://docs.python.org/pt-br/3/library/locale.html#locale.getpreferredencoding[documentação] diz (em parte):
...
https://docs.python.org/pt-br/3/library/locale.html?highlight=strxfrm#locale.strxfrm[documentação do
...
antes de usar pass:[<span class='keep-together'><code>locale.strxfrm</code></span>] como a chave de ordenação.
...
Mesmo assim, ela depende das configurações do locale, criando dores de cabeça na implantação.
...
A `pyuca` não leva o locale em consideração. Se você precisar personalizar a ordenação,
...
A <<cf_demo_fig>> demonstra o script de comando de linha _cf.py_,
...
Veja a documentação da biblioteca padrão para o https://docs.python.org/pt-br/3/library/unicodedata.html[módulo `unicodedata`].
...
(veja https://docs.python.org/pt-br/3/reference/lexical_analysis.html#string-literal-concatenation["2.4.2. String literal concatenation"
...
https://docs.python.org/pt-br/3/library/re.html[documentaçào do módulo `re`] para mais detalhes.
...
Usamos o banco de dados do Unicode para programar um utilitário de comando de linha que
...
O https://docs.python.org/pt-br/3/howto/unicode.html["Unicode HOWTO"]
...
https://fpy.li/4-38["Python 3 and ASCII Compatible Binary Protocols" (_Python 3 e os Protocolos Binários Compatíveis com ASCII_)] (EN) e 
...
https://docs.python.org/pt-br/3/library/codecs.html#standard-encodings["Standard Encodings"] (EN),
...
Mas se você é uma professora no Brasil, seus alunos vão achar mais fácil ler código com variáveis e nomes de função em português,
e escritos corretamente.
...
Para((("Soapbox sidebars", "plain text"))) qualquer um que lide diariamente com texto em línguas diferentes do inglês
 "texto puro" não significa "ASCII". O https://fpy.li/4-55[Glossário do Unicode] (EN) define((("plain text"))) _texto puro_ dessa forma:
...
onde a maioria dos bytes representa valores binários empacotados,
...
usando um número fixo de bytes por ponto de código, para permitir um acesso direto eficiente a qualquer caractere ou fatia.
