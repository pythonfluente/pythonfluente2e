...
O Python 3 introduziu uma forte distinção entre strings de texto humano e sequências de bytes puros. A conversão implícita((("implicit conversion"))) de sequências de bytes para texto Unicode é uma coisa do passado. Este capítulo trata de strings Unicode, sequências binárias, e da codificação usada para converter umas nas outras.

Dependendo do tipo do seu trabalho com o Python,
você pode pensar que entender o Unicode não é importante.
Isso é improvável, mas mesmo que seja o caso, não há como escapar da cisão entre `str` e `byte`.
Como um bônus, você vai descobrir que os tipos especializados de sequências binárias oferecem recursos que o  "pau para toda obra" `str` do Python 2 não possuia.
...
* Ordenação adequada de texto Unicode com `locale`e a biblioteca _pyuca_
...
O suporte((("Unicode text versus bytes", "significant changes to"))) ao Unicode no Python 3 sempre foi muito completo e estável, então o acréscimo mais notável é a seção <<finding_chars_sec>>,
descrevendo um utilitário para busca no banco de dados Unicode—uma forma ótima de encontrar dígitos rodeados por um círculo e gatos sorridentes a partir da linha de comando.

Uma pequena mudança que vale à pena mencionar é o suporte a Unicode no Windows,
que ficou melhor e mais simples desde o Python 3.6, como veremos na seção <<encoding_defaults>>.
...
Em 2023, a melhor definição de "caractere" que temos é um caractere Unicode. Consequentemente, os itens que compõe um `str` do Python 3 são caracteres Unicode, igual aos itens de um objeto `unicode` no Python 2—e não os bytes brutos de uma `str` do Python 2.

O padrão Unicode separa explicitamente a identidade dos caracteres de representações binárias específicas:

[role="pagebreak-before less_space"]
* A identidade de um caractere—((("code points")))seu __ponto de código__&#x2014;é um número de 0 a 1.114.111 (na base 10),
representado no padrão Unicode na forma de 4 a 6 dígitos hexadecimais precedidos pelo prefixo "U+", de U+0000 a U+10FFFF.
Por exemplo, o ponto de código para a letra A é U+0041, o símbolo do Euro é U+20AC, e o símbolo musical da clave de sol corresponde ao ponto de código U+1D11E.
Cerca de 13% dos pontos de código válidos tem caracteres atribuídos a si no Unicode 13.0.0, a versão do padrão usada no Python 3.10.0b4.
* Os bytes específicos que representam um caractere dependem da((("encoding", "definition of"))) _codificação_ (_"encoding"_) usada.
...
Em um outro exemplo, o UTF-8 exige três bytes&#x2014;`\xe2\x82\xac`&#x2014;para codificar o símbolo do Euro (U+20AC),
mas no UTF-16LE o mesmo ponto de código é codificado com dois bytes: `\xac\x20`.
...
Os((("Unicode text versus bytes", "byte essentials", id="UTVbytes04"))) novos tipos de sequências binárias são diferentes do `str` do Python 2 de inúmeras formas.
...
introduzido há tempos, no Python 2.6footnote:[O Python 2.6 e o 2.7 também tinham um `bytes`, mas ele era só um apelido para o tipo `str`.]. A documentação do Python algumas vezes usa o termo genérico "byte string" (_string de bytes_, na documentação em português) para se referir a `bytes` e `bytearray`.
Vou evitar essa expressão confusa.
...
<4> Não há uma sintaxe literal para `bytearray`: elas aparecem como `bytearray()` com um `bytes` literal como argumento.
...
Para todos os outros tipos de sequência no Python, 1 item não é o mesmo que uma fatia de tamanho 1.
====
...
[[ex_buffer_demo]]
.Inicializando bytes a partir de dados brutos de uma array
====
[source, pycon]
----
>>> import array
...
----
====
<1> O typecode `'h'` criar uma `array` de _short integers_ (inteiros de 16 bits).
...
que voce pode usar como o argumento de codificação em funções como
...
afirma que 97% dos sites usam UTF-8, um grande avanço sobre os 81,4% de setembro de 2014, quando escrevi a primeira edição desse livro.
...
.O hack do Leo para advinhar uma decodificação UTF-8
...
Se você tem o requerimento específico de exportar dados para aplicativso que precisam do BOM, use o UTF-8-SIG, mas esteja ciente do que diz a
...
[TIP]
====
Não abra arquivos de texto no modo binário, a menos que seja necessário analisar o conteúdo do arquivo para determinar sua codificação—e mesmo assim, você deveria estar usando o Chardet em vez de reinvertar a roda (veja a seção <<discover_encoding>>).
...
Isso significa que um script como o <<ex_stdout_check>> funciona quando está escrevendo no console, mas pode falhar quando a saida é redirecionada para um arquivo.
...
Ao preparar texto para busca ou indexação, há outra operaçào útil:  _case folding_ (NT: algo como "dobra" ou "mudança" de caixa), nosso próximo assunto.
...
Portanto, a solução da biblioteca padrão para ordenação internacionalizada funciona, mas parece ter surporte adequado apenas no GNU/Linux (talvez também no Windows, se você for um especialista). Mesmo assim, ela depende das configurações do locale, criando dores de cabeça na implantação.

Felizmente, há uma solução mais simples: a((("pyuca library"))) biblioteca _pyuca_, disponivel no _PyPI_.
...
.PyICU: A recomentdação do Miro para ordenação com Unicode
[TIP]
====
...
<3> Texto Unicode para ser usado na busca, contendo os dígitos tamil para `1729` (a linha lógica continua até o símbolo de fechamento de parânteses).
...
O Unicode fornece múltiplas formas de representar alguns caracteres, então a normalização é um pré-requisito para a comparação de textos. Além de explicar a normalização e o _case folding_, apresentamos algumas funções úteis que podem ser adapatadas para as suas necessidades, incluindo transformações drásticas como a remoção de todos os acentos. Vimos como ordenar corretamente texto Unicode, usando o módulo padrão `locale`—com algumas restrições—e uma alternativa que não depende de complexas configurações de locale: a biblioteca((("pyuca library"))) externa _pyuca_.
