...
[[def_is_prime_ex]]
.primes.py: uma verificação de números primos fácil de entender, do exemplo em https://docs.python.org/pt-br/3/library/concurrent.futures.html#processpoolexecutor-example[pass:[<code>ProcessPool&#x200b;Executor</code>] na documentação de Python]
====
[source, py]
----
...
projetada para frameworks Python assíncronos para programação web, tais como _aiohttp_, _Sanic_, _FastAPI_, etc.,
...
O capítulo trata dos benefícios e das desvantagens das threads—com citações convincentes de várias fontes abalizadas—deixando claro que os desafios fundamentais das threads não tem relação com Python ou a GIL.
