...
O SO administra centenas de processos, se assegurando que cada um tenha a oportunidade de progredir, mesmo se a CPU em si não possa fazer mais que quatro coisas ao mesmo tempo.
...
(_distributed task queues_) de vários desenvolvedores—todos capazes de melhorar o desempenho e a escalabilidade de aplicações Python.
...
Por isso o YouTube, o DropBox, o Instagram, o Reddit e outros foram capazes de atingir alta escalabilidade quando começaram,
...
Essas opções familiares não estão disponíveis quando você inicia uma thread ou um processo:
...
Além disso, criar uma thread ou um processo não é barato, você não quer iniciar uma delas apenas para executar uma única computação e desaparecer.
...
Como fazer terminar um "worker" quando ele não é mais necessário?
