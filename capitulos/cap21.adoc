...
++++
<blockquote>
    <p>O problema com as abordagens usuais da programação assíncrona é que elas são propostas do tipo "tudo ou nada". Ou você reeescreve todo o código, de forma que nada nele bloqueie [o processamento] ou você está só perdendo tempo.</p>
...
funções geradoras assíncronas, compreenssões assíncronas, e expressões geradoras assíncronas. Para realçar o fato daqueles recursos da linguagem não estarem limitados ao _asyncio_, veremos um exemplo reescrito para usar a _Curio_—o elegante e inovador framework inventado por David Beazley.
...
Novas seções tratam de recursos da linguagem inexistentes naquele momento, como corrotinas nativas, `async with`, `async for`, e os objetos que suportam esses artefatos.

As ideias na seção <<how_async_works_and_does_not_sec>> refletem lições importantes tiradas da experiência prática, e a considero uma leitura essencial para qualquer um trabalhando com programação assíncrona. Elas podem ajudar você a evitar muitos problemas—seja no Pyhon, seja no Node.js.
...
Imagine((("asyncio package", "example script", id="APexample21")))((("asynchronous programming", "asyncio script example", id="APRscript21"))) que você esteja prestes a lançar um novo blog sobre Python, e planeje registrar um domínio usando uma palavra-chave do Python e o sufixo _.DEV_—por exemplo, _AWAIT.DEV._
...
Caso contrário, o domínio não foi resolvido e pode estar disponível.footnote:[`true.dev` está disponível por US$ 360,00 ao ano quando essas linhas estão sendo escritas. Também notei que `for.dev` está registrado, mas seu DNS não está configurado.]

No _blogdom.py_, a sondagem de DNS é feita via objetos corrotinas nativas.
...
Na verdade, o tempo total é quase o igual ao da resposta mais lenta do DNS, em vez da soma dos tempos de todas as respostas.
...
Fazendo isso, você vai perceber que as corrrotinas podem ser lidas como as boas e velhas funções sequenciais.
...
e que “`await` aó aceita um esperável.”
...
O seu código fica entre a biblioteca _asyncio_ e as bibliotecas assíncronas que você estiver usando, como por exmeplo a _HTTPX_. Isso está ilustrado na <<await_channel_fig>>.
...
Essa é a razão da escolha da epígrafe desse capítulo, que incluí o seguinte conselho:
 "[Ou] você reeescreve todo o código, de forma que nada nele bloqueie [o processamento] ou você está só perdendo tempo.""
...
.Código exemplo da documentaçào do driver PostgreSQL _asyncpg_ 
====
[source, python3]
----
...
Aqui vamos descrever esta última.

Um `asyncio.Semaphore` tem um contador interno que é decrementado toda vez que usamos `await` no método corrotina  `.acquire()`, e incrementado quando chamamos o método `.release()`—que não é uma corrotina porque nunca bloqueia. O valor inicial do contador é estabelecido quando o `Semaphore` é instanciado:
...
Invocar `await` em `.acquire()` não causa qualquer atraso se o contador for maior que zero. Se o contador for 0, entretanto, `.acquire()` suspende a a corrotina que chamou `await` até que alguma outra corrotina chame `.release()` no mesmo `Semaphore`, incrementando assim o contador.
Em vez de usar esses métodos diretamente, é mais seguro usar o `semaphore` como um gerenciador de contexto assíncrono, como fiz na função `download_one` em <<flags2_asyncio_top>>:
...
O driver assíncrono para o MomgoDB https://fpy.li/21-23[Motor] tem uma API compatível com `async/await` que na verdade é uma fachada, encobrindo um núcleo de threads que conversa com o servidor de banco de dados.
...
A lógica de busca no Unicode nesses exemplos é a classe `InvertedIndex` no módulo _charindex.py_ no https://fpy.li/code[repositório de código do _Python Fluente_]. Não há nada concorrente naquele pequeno módulo, então vou dar apenas um explicaçào breve sobre ele, no box opcional a seguir. Você pode pular para a implementação do servidor HTTP na seção <<fastapi_web_service_sec>>.

.Conhecendo o índice invertido
****

Um((("inverted indexes"))) índice invertido normalmente mapeia palavras a documentos onde elas ocorrem. Nos exemplos _mojifinder_, cada "documento" é um caractere Unicode.
A classe `charindex.InvertedIndex` indexa cada palavra que aparece no nome de cada caractere no banco de daos Unicode, e cria um índice invertido em um `defaultdict`.
Por exemplo, para indexar o caractere U+0037—DIGIT SEVEN—o inicializador de `InvertedIndex` anexa o caractere `'7'` aos registros sob as chaves `'DIGIT'` e `'SEVEN'`.
Após indexar os dados do Unicode 13.0.0 incluídos no Python 3.9.1, `'DIGIT'` está mapeado para to 868 caracteres, e `'SEVEN'` para  143, incluindo U+1F556—CLOCK FACE SEVEN OCLOCK e
U+2790—DINGBAT NEGATIVE CIRCLED SANS-SERIF DIGIT SEVEN (que aparece em muitas listagens de código neste livro).
...
.Schema OpenAPI gerado automaticamene para o ponto de acesso `/search`.
...
----
====
<1> Este `await` rapidamente recebe um instância de `asyncio.Server`, um servidor TCP baseado em soquetes. Por default, `start_server` cria e inicia o servidor, então ele está pronto para receber conexões.
<2> O primeiro argumento para `start_server` é `client_connected_cb`, um callback para ser executado quando a conexão com um novo cliente se inicia. O callback pode ser uma função ou uma corrotina, mas precisa aceitar exatamente dois argumentos: um `asyncio.StreamReader` e um `asyncio.StreamWriter`. Entretanto, minha corrotina `finder` também precisa receber um `index`, então usei `functools.partial` para vincular aquele parâmetro e obter um chamável que receber o leitor (`asyncio.StreamReader`) e o escritor (`asyncio.StreamWriter`). Adaptar funções do usuário a APIs de callback é o caso de uso mais comum de `functools.partial`.
...
<5> Mostra o endereço e a porta do primeiro soquete do servidor.
...
<3> Obtém o endereço do cliente remoto ao qual o soquete está conectado.
...
<16> Espera até `StreamWriter` fechar. Isso é recomendado na https://docs.python.org/pt-br/3/library/asyncio-stream.html#asyncio.StreamWriter.close[documentaçào do método `.close()`].
...
Para experimentar com o _domainlib.py_, vá ao diretório _21-async/domains/asyncio/_ na sua cópia local do https://fpy.li/code[repositório de sódigo do _Python Fluente_].
Aí rode::
...
O único artefato definido na PEP 530 que pode aparecer fora do corpo de uma `async def` é uma expressão geradora assíncrona.

===== Definindo e usando uma expressão geradora asséncrona
...
O restante dos artefatos introduzidos pela PEP 530 só podem ser definidos e usados dentro de corrotinas nativas ou de funções geradoras assíncronas.
...
=== async além do asyncio: Curio

Os elementos da linguagem((("asynchronous programming", "Curio project", id="APRcurio21")))((("Curio project", id="curio21"))) `async/await` do Python não estão presos a nenhum loop de eventos ou biblioteca específicos.footnote:[Em contraste com o Javascript, onde `async/await` são parte do loop de eventos e do ambiente de runtime, isto é, um navegador, o Node.js ou o Deno.]
...
Um `TaskGroup` do _Curio_ é um gerenciador de contexto assíncrono que substitui várias APIs e padrões de codiificação ad hoc do `asyncio`.
...
Isso é análogo à programaçào estruturada, que eliminou o comando `GOTO` e introduziu os comandos de bloco para limitar os pontos de entrada e saída de loops e subrrotinas.
...
O primeiro passo é reconhecer que um "sistema voltado para E/S" é uma fantasia.

[[myth_iobound_sec]]
==== O mito dos sistemas voltados para E/S

Um((("asynchronous programming", "myth of I/O-bound systems")))((("network I/O", "myth of I/O-bound systems"))) meme exaustivamente repetido é que programação assíncrona é boa para "sistemas voltados para E/S." Aprendi da forma mais difícil que não existem "sistemas voltados para E/S." Você pode ter _funções_ voltadas para E/S. Talvez a maioria das funções no seu sistema sejam voltadas para E/S; isto é, elas passam mais tempo esperando pela E/S que processando dados. Enquanto esperam, cedem o controle para o loop de eventos, que pode então acionar outras tarefas pendentes. Mas, inevitavelmente, qualquer sistema não-trivial terá partes voltadas para a CPU. Mesmo sistemas triviaiis revelam isso, sob esforço. No <<async_soapbox>>, conto a história de dois programas assíncronos que se debatem com funções voltadas para uso da CPU que desaceleram o loop de eventos, com severos impactos no desempenho.

Dado que qualquer sistema não-trivial terá funções voltadas para uso da CPU, lidar com elas é a chave do sucesso na programação assíncrona.
...
* Decidir que você pode suportar a perda de desempenho e não fazer nada—mas registre essa decisão, para torná-la mais fácil de reverter no futuro.
...
A última opção—não fazer nada—cai na categoria de https://pt.wikipedia.org/wiki/D%C3%ADvida_tecnol%C3%B3gica[dívida tecnológica].

Programação concorrente é um tópico fascinante, e gostaria de escrever muito mais sobre isso. Mas esse não é o foco principal deste livro, e este já é um dos capítulos mais longos, então vamos encerrar por aqui.
...
Por fim, as seções sob o título <<how_async_works_and_does_not_sec>> discutiram o principal atrativo da programação assíncrona, o equívoco de "sistemas voltados para E/S" e como lidar com as inevitáveis partes de uso intesivo de CPU em seu programa.
...
https://fpy.li/21-80["Python async frameworks—Beyond developer tribalism" (_Fremeworks assíncronas do Python—Para além do tribalismo dos desenvolvedores_)] (EN), de Tom Christie.
...
Entretanto, o post revela que a _uvloop_ é capaz de competir com o desempenho do Go sob duas condições:
 ...
 . Que o Go seja configurado para usar uma única thread. Isso faz o runtime do Go se comportar de forma similar so _asyncio_: a concorrência é obtida através de múltiplas corrotinas acionadas por um loop de eventos, todos na mesma thread.footnote:[Usar uma única thread era o default até o lançamento do Go 1.5. Anos antes, o Go já tinha ganho uma merecida reputação por permitir a criação de sistemas em rede de alta concorrência. Mais uma evidência que a concorrência não exige múltiplas threads ou múltiplos núcleos de CPU.]
. Que o código Python 3.5 use o https://fpy.li/21-85[_httptools_] além do próprio _uvloop_.

Selivanov explica que escreveu _httptools_ após testar o desempenho da _uvloop_ com a https://fpy.li/21-86[_aiohttp_]—uma das primeira bibliotecas HTTP completas construídas sobre o `asyncio`:

[quote]
____
Entretanto, o gargalo de desempenho no _aiohttp_ estava em seu parser de HTTP, que era tão lento que pouco importava a velocidade da biblioteca de E/S subjacente. Para tornar as coisas mais interessantes, criamos uma vinculação de Python para a _http-parser_ (a biblioteca em C do parser do Node.js, originalmente desenvolvida para o _NGINX_). A biblioteca é chamada _httptools_, e está disponível no Github e no PyPI.
____
...
A maioria dos desenvolvedores consideraria um simples servidor eco um "sistema vinculado a E/S", certo?
...
Em((("Soapbox sidebars", "Twisted library")))((("Twisted library"))) vez de um simples servidor eco, imagine um sistema Python complexo e em evolução, com milhares de linhas de código assíncrono, e conectado a muitas bibliotexas externas. Anos atrás me pediram para ajudar a diagnosticar os problemas de desempenho de um sistema assim. Ele era escrito em Python 2.7, com a framework https://fpy.li/21-88[_Twisted_]—uma biblioteca sólida e, de muitas maneiras, uma precursora do próprio `asyncio`.

Python era usado para construir uma fachada para a interface de usuário web, integrando funcionalidades fornecidas por biliotecas pré-existentes e ferramentas de linha de comando escritas em outras linguagens—mas não projetadas para execução concorrente.
...
O que estava acontecendo: a cada novo recurso adicional, mais código de uso intensivo de CPU estava desacelerando o loop de eventos do _Twisted_.
O papel do Python como um linguagem de "cola" significava que havia muita interpretação de dados e muitas conversões  entre formatos.
Não havia um gargalo único: o problema estava espalhado por incontáveis pequenas funções criadas ao longo de meses de desenvolvimento.
O conserto exigiria repensar a arquitetura do sistema, reescrever muito código, provavelmente usar um fila de tarefas, e talvez usar microserviços ou bibliotecas personalizadas, escritas em linguagens mais adequadas a processamento concorrente de uso intensivo de CPU.
Os apoiadores internos interessados não estavam preparados para fazer aquele investimento adicional, e o projeto foi cancelado logo em seguida.

Quando contei essa história para Glyph Lefkowitz—fundador do projeto _Twisted_—ele me disse que uma de suas prioridades, no início de qualquer projeto envolvendo programação assíncrona, é decidir que ferramentas serão usadas para a "exportação" de tarefas de uso intensivo de CPU. Essa conversa com Glyph foi a inspiração para a seção <<avoid_cpu_trap_sec>>.
****
