...
Os exemplos de pass:[<em>flags_.py</em>] aqui e no <<futures_ch>> compartilham código e dados, então os coloquei juntos no diretório
...
O seu código delega para as corrotinas do `httpx` explicitamente, usando `await`, ou implicitamente, usando os métodos especiais dos gerenciadores de contexto assíncronos, tais como pass:[<code>Async&#x200b;Client</code>] e `ClientResponse`—como veremos na <<async_context_manager_sec>>.
...
[NOTE]
====
A <<async_gen_context_mngr_sec>> mostra como usar a `contextlib` do Python para criar um gerenciador de contexto assíncrono sem precisar escrever uma classe. Essa explicação aparece mais tarde nesse capítulo por causa de um pré-requsito: a <<async_gen_func_sec>>.
====
...
Após os exemplos simples com o _spinner_ no <<concurrency_models_ch>>,
...
O último exemplo do capítulo foi o _blogdom.py_ reescrito com o framework _Curio_, demonstrando como os recursos de programação assíncrona do Python não estão presos ao pacote `asyncio`.
