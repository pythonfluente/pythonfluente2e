...
Isso é uma boa prática, pois durante uma seção de depuração podemos ver a diferença entre `Vector(1, 2)` e `Vector('1', '2')`. Este objeto não funcionaria no contexto desse exemplo, porque nosso código espera que os argumentos do construtor sejam números, não `str`.
