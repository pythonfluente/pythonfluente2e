...
Uma metaclasse pode  sobrescrever este método, para personalziar a ordem de resolução de métodos da classe em construção.
...
Vamos estudar esse método especial na <<enhancing_with_init_subclass>>, mas não vou tratar do uso de argumentos nomeados. Para saber mais sobre isso, por favor leia a https://fpy.li/pep487[PEP 487—Simpler customization of class creation (_PEP 487—Uma personalização mais simples da criação de classes_)] (EN).
====
...
Vamos criar metaclasses personalizadas na <<metclass101_sec>>.
