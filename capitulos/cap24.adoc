...
O `@dataclass` de Python faz muito mais.
...
Além de `@dataclass`, um exemplo interessante—e muito mais simples—de decorador de classes na bilbioteca padrão de Python é https://docs.python.org/pt-br/3/library/functools.html#functools.total_ordering[`functools.total_ordering`] (EN), que gera métodos especiais para comparação de objetos.
...
Melhor ainda, os recursos avançados estão integrados de forma a não afetar a adequação de Python para programação casual (eles na verdade ajudam nisso, por trás da cortina).
