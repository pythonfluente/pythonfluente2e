...
Mas daí, se você der tudo de si ao escrever o programa, como vai conseguir depurá-lo?footnote:[Citação extraída do capítulo 2, _Expression_ ("Expressão"), página 10, de _The Elements of Programming Style ("Elementos de Estilo de Programação"), Segunda Edição_. (NT: Não há edição traduzida deste livro disponível.)]
____
...
Vamos começar revisando os atibutos e métodos definidos no Modelo de Dados do Python para todas as classes.
...
Toda classe tem alguns atributos definidos no Modelo de Dados do Python, documentados na seção https://docs.python.org/pt-br/3/library/stdtypes.html#special-attributes["4.13. Atributos Especiais"] do capítulo "Tipos Embutidos" da _Bibloteca Padrão do Python_.
...
[NOTE]
====
O construtor `type` aceita argumentos nomeados opcionais, que são ignorados por `type`, mas que são passados como recebidos para `+__init_subclass__+`, que deve consumí-los.
...
Acreditamos em _falhar rápido_ como melhor prática, então não queremos ignorar silenciosamente os itens em excesso. Uma solução rápida e eficiente é extrair um item de `kwargs` e tentar definí-lo na instância, gerando propositalmente um `AttributeError`.
...
Decoradores de classes:: Mais simples de entender que metaclasses, e com menor probablidade de causar conflitos com classes base e metaclasses.
...
- Persistência baseda em objetos
...
Outra ideia poderosa são as funções como objetos de primeira classe, pavimentando o caminho para funções de ordem superior. E acontece que a combinação de descritores e funções de ordem superior permite a unificação de funções e métodos. O `+__get__+` de uma função produz um objeto método em tempo real, vinculando a instância ao argumento `self`. Isso é elegante.footnote:[__Machine Beauty: Elegance and the Heart of Technology__ ("Beleza de Máquina: A Elegância e o Coraçào da Tecnologia"), de David Gelernter (Basic Books), começa com uma discussão intrigante sobre elegância e estética em obras de engenharia, de pontes a software. Os capítulos posteriores não são tão bons, mas o início vale o preço.]
...
Aguardo ansiosamente notícias sobre suas contribuições ao ecossistema e à comunidade do Python!
****
