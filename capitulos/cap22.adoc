...
O método `+Record.__init__+` ilustra um antigo truque de Python. Lembre-se que o `+__dict__+` de um objeto é onde são mantidos seus atributos--a menos que `+__slots__+` seja declarado na classe, como vimos na <<slots_section>>.
...
Vimos `@cache` e `@lru_cache` na <<memoization_sec>> (no <<ch_closure_decorator>>). Python 3.8 introduziu `@cached_property`.
...
Revise a <<slots_section>> para recordar esse tópico.
