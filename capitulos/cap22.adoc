...
Vimos inicialmente o decorador `@property` no <<pythonic_objects>>, na <<hashable_vector2d>>. No <<ex_vector2d_v3>>, usei duas propriedades no `Vector2d` apenas para tornar os atributos `x` e `y` apenas para leitura.
...
e o <<attribute_descriptors>> vai revelar que uma propriedade é implementada como um descritor—uma abstração mais geral e poderosa.
====
...
Vimos `@cache` e `@lru_cache` na <<memoization_sec>> (no <<closures_and_decorators>>). Python 3.8 introduziu `@cached_property`.
