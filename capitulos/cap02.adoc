...
++++
<blockquote>
<p>Como vocês podem ter notado, várias das operações mencionadas funcionam da mesma forma com textos, listas e tabelas. Coletivamente, textos, listas e tabelas são chamados de 'trens' (<em>trains</em>). [...] A instrução `FOR` também funciona, de forma geral, em trens.</p>
...
E no console de Python, o resultado da instrução anterior é atribuído a `+_+`—a menos que o resultado seja `None`.
====
...
que alguns projetistas de linguagens copiaram irracionalmente do C—décadas após sabermos que tais problemas são a causa de inúmeros bugs.]
...
O((("pattern matching", "&#x002A;&#x005F; symbol")))((("&#x002A;&#x005F; symbol"))) `+*_+` casa com qualquer número de itens,
sem vinculá-los a uma variável.
...
Peter Norvig((("lis.py interpreter", "pattern matching in", id="lispypattern02")))((("pattern matching", "in lis.py interpreter", secondary-sortas="lis.py", id="PMinterp02")))((("Scheme language", id="scheme02"))),
...
métodos especiais  `+__getitem__+` e `+__setitem__+`, que tratam o operador `[]`, apenas recebem os índices em `a[i, j]` como uma tupla.
...
então aceitam apenas um índice ou fatia, e não uma tupla de índices ou fatias.footnote:[Na <<memoryview_sec>> vamos mostrar que
...
Sequências((("slicing", "assigning to slices"))) mutáveis podem ser transplantadas,
extirpadas e, de forma geral, modificadas no lugar com o uso da notação de fatias
