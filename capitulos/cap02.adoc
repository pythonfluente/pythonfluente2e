...
____
Como vocês podem ter notado, várias das operações mencionadas funcionam da mesma forma com textos, listas e tabelas.
...
parâmetros de função com `*args` para capturar argumentos arbitrários
...
_greatest common divisor) dos argumentos (que é 9).
...
todo o bloco `match` não faz nada quando o sujeito não casa com algum `case`—e isso pode ser uma falha silenciosa.
...
Os melhores argumentos a favor desta convenção foram escritos pelo cientista da computação holandês
Edsger W. Dijkstra (veja a última referência na <<array_fur_reading_sec>>).
...
apenas recebem os índices em `a[i, j]` como uma tupla.
...
então aceitam apenas um índice ou fatia, e não uma tupla de índices ou
...
Programadores((("sequences", "using &#x002B; and &#x002A; with", id="Splusast02")))((("&#x002B; operator",
id="plusop02")))((("star (&#x002A;) operator", id="starseq02")))((("&#x002A; (star) operator",
id="starseq02a")))((("concatenation", id="concat02")))
Python esperam que sequências suportem `\+` e `*`.
...
vai resultar em uma lista com três referências para a mesma lista interna, que provavelmente não é o quê você quer.
====
...
O((("&#x005F;&#x005F;iadd&#x005F;&#x005F;"))) método especial que faz `+=` funcionar é `\__iadd_\_`
...
`a` será modificada no lugar (isto é, o efeito ser similar a `a.extend(b)`).
...
Após testemunharmos as sutilezas do uso de `+` e `*` para concatenação,
...
e que os caracteres não-ASCII dificilmente serão ordenados de forma razoável.
...
80.000.000 bytes (8 bytes por número, zero excesso),
...
Por((("NumPy", id="numpy02"))) todo esse livro, procuro destacar o que já existe na biblioteca padrão de Python,
para que você a aproveite ao máximo.
Mas a NumPy é tão importante que exige um desvio.

Por suas operações avançadas de arrays e matrizes,
o Numpy é a razão pela qual Python se tornou uma das principais linguagens para aplicações de computação científica.
...
salvar e operar sobre todos os elementos de uma `numpy.ndarray`:
...
nos voltar para um grupo completamente diferentes de substitutos para a boa e velha `list`:
...
uma fila de duas pontas e segura para usar com threads,
projetada para inserção e remoção rápida nas duas pontas.
