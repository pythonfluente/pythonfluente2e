...
[quote, Tim Peters, lendário desenvolvedor principal do Python e autor do <em>O Zen do Python</em>]
____
...
o principal público deste livro não deve ter qualquer problema em
...
Pense nas partes <<data_structures_part>> até <<metaprog_part>> como cinco livros dentro do livro.
...
pass:[<a data-type="xref" href="#data_structures_part" data-xrefstyle="chap-num-title">#data_structures_part</a>]:: 
O <<data_model>> introduz o Modelo de Dados do Python e explica porque os métodos especiais (por exemplo, `+__repr__+`) são a chave do comportamento consistente de objetos de todos os tipos. Os métodos especiais são tratatos em maiores detalhes ao longo do livro. Os((("data structures"))) capítulos restantes dessa parte cobrem o uso de tipos coleção: sequências, mapeamentos e conjuntos, bem como a separação de `str` e `bytes`--causa de muitas celebrações entre usuários do Python 3, e de muita dor para usuários de Python 2 obrigados a migrar suas bases de código. Também são abordadas as fábricas de classe de alto nível na biblioteca padrão: fábricas de tuplas nomeadas e o decorador `@dataclass`. _Pattern matching_ ("Correspondência de padrões")—novidade no Python 3.10—é tratada em seções do pass:[<a href="#sequences" data-type="xref" data-xrefstyle="select:labelnumber">#sequences</a>], do pass:[<a href="#dicts-a-to-z" data-type="xref" data-xrefstyle="select:labelnumber">#dicts-a-to-z</a>] e do pass:[<a href="#data_class_ch" data-type="xref" data-xrefstyle="select:labelnumber">#data_class_ch</a>], que discutem padrões de sequências, padrões de mapeamentos e padrões de classes.
...
=== Website do livro: fluentpython.com

Tratar((("fluentpython.com")))((("Python", "fluentpython.com"))) dos novos recursos—como dicas de tipo, classes de dados e _pattern matching_—tornaram essa segunda edição 30% maior que a primeira.
Para preservar a posssibilidade de levar o livro na bagagem, transferi parte do conteúdo para 
http://fluentpython.com[_fluentpython.com_].
Há links para artigos publicados ali em vários capítulos. Alguns capítulo de amostra também estão no site.
O texto completo em português desta segunda edição está https://github.com/pythonfluente/pythonfluente2e[disponível online]
...
_Itálico_:: Indica novos termos, URLs, endereços de email, nomes e extensões de arquivos [NT: E nesta edição em português, em certos contextos, termos mantidos em inglês ou traduções de termos cuja versão em português não é familiar].
...
ele não funciona porque é uma referência para um âncora não definida.
Precisamos corrigir.
