...
Como veremos na <<how_slicing_works_sec>>, para avaliar a expressão `seq[start:stop:step]`,
...
|      `s.frombytes(b)` |       |   ●   | Acrescenta itens de uma sequência de bytes, interpretada como valores em código de máquina compactos
|    `s.fromfile(f, n)` |       |   ●   | Acrescenta `n` itens de um arquivo binário `f`, interpretado como valores em código de máquina compactos
