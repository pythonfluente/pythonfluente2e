...
A <<pattern_matching_case_study_sec>> é nova: um exemplo maior de casamento de padrões.
...
A nota `①` no <<with_file_demo>> é sutil mas muito importante:
...
instrução `async with`, tratada no <<ch_async>>.
...
de Martijn Pieters. O <<inplace_ex>> mostra como usar
...
que seu código leia e escreva nele ao mesmo tempo. Isso é mais fácil de usar que
...
`(define ...)`, `(if ...)` e `(quote ...)`, que de têm comportamentos muito
...
O <<ex_gcd_python>> é o mesmo código em Python (mais curto que a explicação em português do
...
depois de cada parênteses na entrada, e então a particiona com `split`, resultando em uma lista
...
mas não é amigável para usuário (veja o <<ex_lispy_repl>>). Se nenhum argumento de
...
continuação, como o REPL do Python qie sabe quando não terminamos uma instrução e apresenta um prompt
...
`lispstr(exp: object) -> str`::
    ...
    `lispstr` devolve o código-fonte correspondete.
...
com `match/case`. A função `evaluate` no <<ex_evaluate_match>> recebe uma
...
`set!` está relacionado ao uso da instrução `nonlocal` em Python:
...
`(define x 10)` sará sempre uma variável local, criada ou atualizada no ambiente
local.
...
casa com qualquer sequência sujeito que tenha um ou mais itens.
Entretanto, se `func_exp` é uma palavra reservada e o
sujeito não casou com nenhum dos `case` anteriores,
então isso é um erro de sintaxe, como `(define x)`.
...
Este é um padrão-OU que já vimos na <<eval_atom_sec>>:
...
independente de qual sub-padrão tenha casado.
...
meu entendimento destes cláusulas, mas no fim acabei me acostumando.
...
acharam que ela também devia ter uma cláusula `else`. No fim ficou
...
o significado da instrução `with` e os gerenciadores de contexto, indo rapidamente
além de uso mais comum, fechar arquivos automaticamente. Implementamos
...
depois de outro procedimento.footnote:[Veja o https://fpy.li/18-29[slide 21 em
...
dentro do bloco `with` é criada um cópia de trabalho da instância de `list`, e
...
Para usos na vida real, não se esqueça que Python tem o
