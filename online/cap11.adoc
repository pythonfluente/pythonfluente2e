...
delimitado por `{}` dentro de uma f-string ou no gabarito `g` em `g.format()`
...
Se a classe não implementar um `+__format__+`,
...
`+__format__+` que analisa uma `fmt_spec` fornecida à função embutida
`format(obj, fmt_spec)` ou dentro de campos de substituição
`'{:«fmt_spec»}'` em f-strings ou ainda strings usadas com o método
`str.format()`.
