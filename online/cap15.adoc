...
[quote]
____
Coerções são usadas para silenciar avisos espúrios do checador de tipos, e dão uma ajuda ao verificador quando ele não consegue entender direito o que está acontecendo.
____
...
E desde Python 3.8, esse _getter_ devolve uma `tuple` (usada como uma sequência imutável).
...
A avaliação das anotações no momento da importação é o padrão desde Python 3.10,
...
[WARNING]
====
Desde Python 3.10, a nova função https://fpy.li/15-25[`inspect.get_annotations(…)`] deve ser usada, em vez de pass:[<code>typing.&#x200b;get_&#x200b;type_&#x200b;hints</code>].
...
* Evite ler `+__annotations__+` diretamente; em vez disso, use `inspect.get_annotations` (desde Python 3.10) ou `typing.get_type_hints` (desde Python 3.5).
