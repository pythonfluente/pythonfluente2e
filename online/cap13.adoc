...
verificar o tipo do objeto (sem nem mencionar a abordagem ainda pior de
verificar, por exemplo, se `type(foo) is bar`&#x2014;que é corretamente
...
Fora dos frameworks, tipagem pato é muitas vezes mais simples e flexível que verificações de tipo.
...
então `obj` não é _hashable_, apesar do resultado positivo da verificação com
`isinstance`.
...
[TIP]
====
Só encontrei resultados inesperados usando verificações com `isinstance` em conjunto com os protocolos numéricos `Supports*` quando fiz experiências de conversão de ou para `complex`.
