...
Tipagem pato estática (_static duck typing_)::
    ...
    Tratamos desse tema pela primeira vez na <<protocols_in_fn_sec>>,
...
mas diferente.
Como vimos na <<protocols_in_fn_sec>>,
...
em https://fluentpython.com para conhecer mais sobre esta função.
====
...
O <<ex_lotto>> mostra uma implementação muito diferente, mas também válida,
...
<<defensive_argument_sec>> trata do problema de apelidamento
...
(veja a <<register_usage>>), mas também pode ser usado como decorador.
No ((("UML class diagrams", "TomboList"))) <<ex_tombolist>>,
usamos a sintaxe de decorador e implementamos `TomboList`,
uma subclasse virtual de `Tombola`, ilustrada na <<tombolist_uml>>.
...
sobre o atributo de classe `+__mro__+` na <<mult_inherit_mro_sec>>. Por agora, essas
...
virtual de `Sized`. Veja o <<sized_source_code>>.
...
id="Pstatic13"))) na <<protocols_in_fn_sec>>. Pensei em deixar toda a discussão
...
como a classe `Vector` aprimorada que veremos na <<overloading_mul_sec>>:
...
`Vector2d`, que desenvolvemos no <<ch_pythonic_obj>>? Dado que tanto um número
...
ganso, vimos a ABC `Tombola` na <<defining_using_abc_sec>>. Aqui vamos ver como
...
mão. Como mencionado na <<protocol_type_hints_ignored>>, o tipo `complex` no
...
A próxima parada foi a <<prog_ducks_sec>>, onde exploramos os esforços que o
...
iniciamos na <<protocols_in_fn_sec>>. Vimos como
