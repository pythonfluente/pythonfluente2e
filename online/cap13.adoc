...
O <<ex_duck_typing_str_list>> mostra uma situação em que a tipagem pato é mais
...
Detalhes sobre o uso de `register` são tratados na <<virtual_subclass_sec>>,
...
mostra a <<mutablesequence_uml>>, nem todos os métodos das ABCs `Sequence`
...
múltipla acontecendo na <<collections_uml>>. Vamos dedicar a maior parte do
