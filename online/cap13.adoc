...
Tipagem pato (_duck typing_)::
    ...
    Estamos estudando tipagem pato desde <<ch_data_model>>.
...
abstratas.
Este capítulo é sobre tipagem pato, goose typing (_tipagem ganso_), e
...
* A <<prog_ducks_sec>> se aprofunda na tipagem pato já familiar para quem programa em Python,
incluindo como fazê-la mais segura, preservando sua melhor
qualidade: a flexibilidade.
...
`numbers` para goose typing. Na <<numbers_abc_proto_sec>> eu explico porque, em
vez disso, você deve usar protocolos numéricos estáticos do módulo `typing` se
...
para permitir checagem de tipo explícita durante a execução (também conhecida
como _goose typing_), bem como para servirem de dicas de tipo para checadores de
tipos estáticos.
...
sequência. O protocolo iterável em Python representa uma forma extrema de duck
...
protocolos dinâmicos sem recorrer a verificações explícitas.((("",
...
`random.shuffle`. Neste caso, uma verificação durante a execução como
...
Código defensivo usando tipos "duck" também podem incluir lógica para lidar com
tipos diferentes sem usar testes com `isinstance()` e `hasattr()`.
...
<1> Supõe que é uma string (EAFP—mais fácil pedir perdão que permissão).
...
que diga "`+field_names+` deve ser uma string de identificadores separados por
espaços ou vírgulas." Esta é a parte relevante da assinatura de `namedtuple` no
typeshed (veja o código-fonte completo em pass:[<a
href="https://fpy.li/13-9"><em>stdlib/3/collections/__init__.pyi</em></a>]):
...
Como se vê, `field_names` está anotado como `Union[str, Iterable[str]]`, que
serve para seus propósitos, mas não é suficiente para evitar todos os problemas
possíveis.

Após revisar protocolos dinâmicos, passamos para uma forma mais explícita de
checagem de tipo durante a execução: goose typing.((("",
...
A goose typing é uma abordagem à checagem de tipo durante a execução que se apoia nas ABCs.
...
Recebi https://fpy.li/13-11[créditos na Wikipedia] por ter ajudado a popularizar o útil meme e a frase de efeito "_duck typing_" (isto é, ignorar o tipo efetivo de um objeto, e em vez disso se dedicar a assegurar que o objeto implementa os nomes, assinaturas e semântica dos métodos necessários para o uso pretendido).
...
No geral, a abordagem da _duck typing_ continua muito útil em inúmeros contextos—mas em muitos outros, um nova abordagem muitas vezes preferível evoluiu ao longo do tempo.
E aqui começa nossa história...

Em gerações recentes, a taxinomia de gênero e espécies (incluindo, mas não limitada à família de pássaros aquáticos conhecida como Anatidae) foi guiada principalmente pela _fenética_—uma abordagem focalizada nas similaridades de morfologia e comportamento... principalmente traços _observáveis_.
A analogia com o "duck typing" era patente.
...
Daí, quando precisar verificar se (tipicamente) um argumento é, por exemplo, "uma sequência", verifique se:
[source, python]
----
...
Como é impossível nomear todos os tipos que implementam um protocolo (suportando as operações requeridas), a duck typing não podia ser descrita por dicas de tipo antes de Python 3.8.
...
<4> Agora o checador de tipos pode verificar que o parâmetro `x` é um objeto que pode ser multiplicado por um inteiro, e que o valor retornado tem o mesmo tipo que `x`.
...
[TIP]
====
Se você estiver usando um checador de tipos externo, há uma vantagem nas verificações explícitas com `isinstance`:
...
Mas eu prefiro aproveitar a tipagem pato e fazer isso usando o princípio
EAFP—_Easier to Ask Forgiveness than Permission_
...
por apontar que checagem de tipo não é apenas uma questão de verificar se o tipo de `x` é `T`: é sobre determinar que o tipo de `x` é _consistente-com_ `T`, o que pode ser caro.
...
os quais suportam duck typing e tipagem pato estática, respectivamente.
...
Dito isso, Python ganhou sua enorme popularidade enquanto suportava apenas duck typing.
...
https://fpy.li/13-42["I Want A New Duck: `typing.Protocol` and the future of duck typing"] (EN).("Eu Quero Um Novo Pato: `typing.Protocol` e o futuro da tipagem pato`").
...
a verificação estática efetiva sendo executada por ferramentas externas.
...
Durante todos esses anos, alguns usuários de Python—incluindo este autor—ficaram
desapontados com o suporte à tipagem. Após aprender Go, a ausência de duck
typing estático em Python era incompreensível, em uma linguagem onde o duck
typing havia sempre sido uma força central.
...
usadas hoje. Eu também coloquei Go no quadrante do goose typing por causa
de sua sintaxe especial para verificação de tipo (_type assertion_),
...
goose typing 20 anos atrás, mas pode ser que existam.
...
Ao contrário de Ruby e Javascript, Python não permite modificações de tipos embutidos durante a execução.
Eu na verdade considero isso uma vantagem, pois dá a certeza que um objeto `str` vai sempre ter os mesmos métodos.
