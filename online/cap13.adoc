...
Duck typing (_tipagem pato_)::
    ...
    Estamos estudando duck typing desde <<ch_data_model>>.
...
abstratas.
Este capítulo é sobre duck typing, goose typing (_tipagem ganso_), e
duck typing estática—disciplinas de tipagem com foco em interfaces.
...
* A <<prog_ducks_sec>> se aprofunda no já familiar duck typing de Python,
incluindo como +
fazê-lo mais seguro e ao mesmo tempo preservar sua melhor
...
importante, mas porque há mais seções sobre duck typing, duck typing estático e
...
* <<two_kinds_protocols_sec>> explica as semelhanças e diferenças entre
protocolos dinâmicos e estáticos.

* <<defensive_duck_prog_sec>> praticamente reproduz o conteúdo da primeira
edição, mas foi atualizada e agora tem um título de seção que enfatiza sua
importância.

* <<static_protocols_sec>> é toda nova. Ela se apoia na apresentação inicial em
...
natureza dinâmica dos protocolos no duck typing dinâmico: `random.shuffle` não
...
O duck typing não precisa ser loucamente inseguro ou difícil de depurar e
...
<<ex_duck_typing_str_list>> mostra como eu faria isso usando duck typing.
...
O <<ex_duck_typing_str_list>> mostra uma situação onde o duck typing é mais
expressivo que dicas de tipo estáticas. Não há como escrever uma dica de tipo
que diga "`field_names` deve ser uma string de identificadores separados por
espaços ou vírgulas." Essa é a parte relevante da assinatura de `namedtuple` no
...
Graças ao duck typing, `double` funciona mesmo com tipos do futuro, tal como a classe `Vector` aprimorada que veremos no <<overloading_mul>> (<<ch_op_overload>>):
...
em vez de chamar `isinstance` ou `hasattr`, apenas tente realizar as operações que você precisa com o objeto, e trate as exceções conforme necessário.
...
Mas eu prefiro aproveitar o duck typing e fazer isso usando o princípio
...
Após((("static protocols", "best practices for protocol design"))) 10 anos de experiência com duck typing estático em Go, está claro que protocolos estreitos são mais úteis—muitas vezes tais protocolos têm um único método, raramente mais que um par de métodos.
...
empresa líder em desenvolvimento ágil de software.
...
os usuários com o objetivo de validar hipóteses centrais do negócio," como
definido or meu ex-colega Paulo Caroli em
...
map"))) uma variação do Mapa de Tipagem(<<type_systems_described>>) com os nomes
de algumas linguagem populares que suportam cada um dos modos de tipagem.

[[type_systems_languages]]
.Quatro abordagens para checagem de tipos e algumas linguagens que as usam.
image::../images/flpy_1308.png[align="center",pdfwidth=10cm]
...
foi a pioneira do duck typing estático—pelo menos entre as linguagens mais
usadas hoje. Eu também coloquei Go no quadrante do goose typing por causa de
suas declarações (_assertions_) de tipo, que permitem a verificação e adaptação
a diferentes tipos durante a execução.

No ano 2000, apenas os quadrantes do duck typing e da tipagem estática tinham
linguagens. Não conheço nenhuma linguagem que suportava duck typing estático ou
