...
apoia nas ABCs. Vou deixar que Alex Martelli explique, no <<waterfowl_essay>>.
...
satisfazer a interface de `Tombola`, mas não há problema em adicionar
métodos.
...
<<ex_lotto>>. A questão é: podemos ser preguiçosos e escolher apenas herdar os
método concretos menos que ideais de uma ABC. Os métodos herdados de `Tombola`
...
Isso é feito invocando um método de classe `register` da ABC, e será reconhecido assim por `issubclass`, mas não implica na herança de qualquer método ou atributo da ABC.
...
Antes de Python 3.3, `register` não podia ser usado dessa forma—ele tinha que ser invocado como uma função normal após a definição da classe, como sugerido pelo comentário no final do <<ex_tombolist>>.
