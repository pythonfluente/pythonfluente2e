...
Vamos começar explicando porque a programação assíncrona é útil,
...
introduz a filosofia por trás de seu projeto dizendo "Estamos fazendo E/S de
...
A <<latency_tbl>> explica porque uma abordagem disciplinada da programação
...
caixa _<<async_soapbox>>_ ao final deste capítulo escrevi sobre dois programas assíncronos sofrendo com
...
A opção deixar como está entra na conta de https://fpy.li/b8[«dívida tecnológica»].
...
Após os exemplos simples com o _spinner_ no <<ch_concurrency_models>>, aqui
...
_concorrência estruturada_, que pode vir a ter um grande impacto em toda a
indústria de tecnologia, trazendo mais clareza para o código concorrente.

Por fim, a <<how_async_works_and_does_not_sec>> apresentou
o principal atrativo da programação assíncrona: não perder tempo
esperando por E/S.
Também vimos que não existem sistema limitados só por E/S,
...
(Qual é a principal diferença entre `asyncio` e _Trio_?) no StackOverflow.

Para aprender mais sobre o pacote `asyncio`, já mencionei os melhores recursos
que conheço no início do capítulo: a https://fpy.li/b9[documentação
oficial], após a https://fpy.li/21-69[«profunda reorganização»] iniciada
...
apresentada por Michael Kennedy, onde entre outras coisas aprendi sobre a biblioteca
...
é também explica esta aplicação do início ao fim.
...
discutindo os modelos de execução incompatíveis de funções comuns versus
...
Go sob duas condições:
 ...
 através de múltiplas corrotinas acionadas por um laço de eventos, todos na
 ...
 criação de sistemas em rede de alta concorrência. Mais uma evidência que a
...
milhares de linhas de código assíncrono, e conectado a muitas bibliotexas
externas.
...
foi talvez o maior erro daquela projeto: as partes interessadas não forçaram uma
abordagem MVP—entregar o "Mínimo Produto Viável" o mais rápido possível e
acrescentar novos recursos em um ritmo estável.] Com o tempo, os desenvolvedores
notaram que o desempenho do sistema como um todo estava diminuindo, e estavam
tendo muita dificuldade em localizar os gargalos.

O que estava acontecendo: cada nova funcionalidade introduzia mais código
intensivo em CPU desacelando o laço de eventos do _Twisted_. O papel de Python
como um linguagem de integração entre processos externos implicava em
...
um fila de tarefas, e talvez usar microsserviços ou bibliotecas customizadas,
...
Glyph foi a inspiração para a <<avoid_cpu_trap_sec>>.
