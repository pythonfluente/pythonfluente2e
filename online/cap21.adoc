...
salvando-o em uma variável ou coleção que você controla.
...
https://fpy.li/weakref[«Weak References»] no _https://fluentpython.com_.footnote:[Agradeço
...
Agora vamos estudar a versão `asyncio` de um script que baixa um imagens de bandeiras da Web.
...
Por exemplo, a corrotina `loop.getaddrinfo(…)`,  que vimos no <<blogdom_ex>> é implementada invocando a função `getaddrinfo` do módulo `socket`—uma função bloqueante que pode levar alguns segundos para retornar, pois depende de resolução de DNS.
