...
forma, milhares de clientes podem ser atendidos de concorrentemente por esse
...
<5> O método `StreamWriter.write` não é uma corrotina, é um função
...
O último trecho deste código é a corrotina `search`, <<tcp_mojifinder_search>>.
...
<3> Esta expressão geradora vai produzir strings de bytes codificadas em UTF-8 com o ponto de código Unicode, o caractere, seu nome e uma sequência `CRLF` (_Return+Line Feed_), isto é, `b'U+0039\t9\tDIGIT NINE\r\n'`.
...
<<async_context_manager_sec>> vimos como `async with` funciona com objetos que
...
assíncrono. No <<blogdom_ex>> vimos _blogdom.py_, um script que sondava nomes de
...
----
====
<1> Experimente um simples `await` para ver o console assíncrono em ação. Dica: `asyncio.sleep()` pode receber um segundo argumento opcional que é devolvido através do `await`.
<2> Acione a corrotina `probe`.
<3> A versão de `probe` em `domainlib` devolve uma `NamedTuple` chamada `Result`.
<4> Faça um lista de domínios. O domínio de nível superior `.invalid` é reservado para testes. Consultas ao DNS por tais domínios sempre recebem uma resposta NXDOMAIN dos servidores DNS, que significa "este domínio não existe."footnote:[Veja https://fpy.li/21-45[_RFC 6761—Special-Use Domain Names_].]
...
O <<domainlib_demo_repl>> mostra que `multi_probe` é um gerador assíncrono, pois é compatível com `async for`. Vamos executar mais alguns experimentos, continuando com o <<domainlib_more_exp_repl>>.

[[domainlib_more_exp_repl]]
.Mais experimentos, continuação do <<domainlib_demo_repl>>
====
[source, python]
----
>>> probe('python.org')  # <1>
...
[NOTE]
====
O corpo do laço `for` no <<domainlib_ex>> poderia ser mais conciso:
...
Graças à _domainlib_, o códido de _domaincheck.py_ é bem direto:
...
o uso de `async for` e `await` na sintaxe de compreensões e expressões geradoras, a partir de Python 3.6.
...
Agora veremos as demais construções sintáticas introduzidos pela PEP 530.
...
Estas construções são muitas vezes usados com o `asyncio` mas, na verdade, 
...
passado. Lembre-se que o `asyncio` foi lançado no Python 3.4, quando não existiam
as instruções `async`, e em vez de `await` usávamos `yield from`.
Então a API original do `asyncio` não podia oferecer gerenciadores de
...
O <<blogdom_curio_ex>> mostra o script _blogdom.py_ (<<blogdom_ex>>) reescrito
para usar o _Curio_.
...
<2> ...`getaddrinfo` é uma função nível superior de `curio.socket`, não um
...
<5> Iterar com `async for` sobre um `TaskGroup` produz instâncias de `Task` a
medida que cada uma termina. Isto corresponde à linha em <<blogdom_ex>> que usa +
`for … in as_completed(…):`
...
entrada e uma saída. Isto é análogo à programaçào estruturada, 
...
a instrução `GOTO` que permitia desviar a execução de qualquer linha
...
`asyncio` em outra thread, no mesmo processo, comunicando-se através da
...
Nesse capítulo vimos vários exemplos de corrotinas nativas anotadas,
incluindo a `probe` do <<blogdom_curio_ex>>:
...
Primeiro: eles são todos covariantes do primeiro parâmetro de tipo, que é o tipo
dos itens produzidos a partir desses objetos. Lembre-se da regra #1 da
<<variance_rules_sec>>:
...
no objeto após sua construção inicial, ele pode ser contra-variante.
____
