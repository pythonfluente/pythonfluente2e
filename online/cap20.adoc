...
`ThreadPoolExecutor` agora também reutiliza threads de trabalho inativas antes
iniciar [novas] threads de trabalho de `max_workers`.
...
A biblioteca se chama `concurrency.futures`, mas não há qualquer _future_ à vista no <<flags_threadpool_ex>>,
então você pode estar se perguntando eles estão.
...
Especificamente, instâncias `concurrent.futures.Future` são criadas como resultado de submeter um objeto invocável (_callable_)
...
Os dois tipos de `Future` tem um método `.done()` não-bloqueante, que devolve um `bool` informando se o invocável encapsulado por aquele `future` foi ou não executado.
...
Observe que aquela função de callback será invocada na mesma thread ou processo de trabalho que rodou a função encapsulada no _future_.
...
Observe que a chamada a `future.result()` nunca bloqueará a thread neste
...
Já 7777777536340681 é 88191709^2^, então `is_prime` vai demorar mais para determinar que é um número composto,
e ainda mais para descobrir que 7777777777777753 é primo. Por isso esses números aparecem próximo do final da saída do _procs.py_ original.
...
Agora vamos entender comportamento estranho de _proc_pool.py_:
...
Entender como programas concorrentes se comportam não é um simples,
...
mensagem quanto inicia, cochilar por `n` segundos, e mostrar uma mensagem quando
...
de _your mileage may vary_, algo como _sua quilometragem pode variar_, querendo
...
A função `Executor.map` é fácil de usar, mas muitas vezes é preferível obter os
resultados assim que estejam prontos, independente da ordem em que foram
submetidos. Para fazer isso, precisamos de uma combinação do método
...
id="EHnetwork20"))) mencionado, os scripts em <<ex_web_downloads_sec>> não
tem tratamento de erros, para torná-los mais fáceis de ler e para comparar a
...
arquivo (`download_one`). Outra exceções são tratadas pela
...
não-intencional contra o Google Maps, para criar um mapa colorido de tempos de
...
concorrência. Erlang não me empolga só por que acho sua sintaxe feia.
...
linguagem inchada quando muitas macros foram incoporadas ao padrão.
