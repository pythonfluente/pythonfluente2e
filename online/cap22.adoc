...
____
Todos os serviços oferecidos por um módulo deveriam estar disponíveis através de uma notação uniforme, que não revele se eles são implementados por armazenamento ou por computação.footnote:[Bertrand Meyer, Object-Oriented Software Construction, 2nd ed. (Pearson), p. 57.]
____
...
A maioria das((("dynamic attributes and properties", "significant changes to")))
atualizações deste capítulo foram motivadas pela discussão relativa a
`@functools.cached_property` (introduzido no Python 3.8), bem como pelo uso
...
a lógica para baixar o conjunto de dados da OSCON—que os exemplos agora leem de
...
A pedra angular da classe `FrozenJSON` é o metodo `+__getattr__+`, que já usamos
...
Se `+self.__data+` não tiver uma atributo com o `name` dado, `+__getattr__+`
...
solução melhor é verificar se uma chave no mapamento passado para
...
[[ex_explore1]]
.explore1.py: anexa um `_` a nomes de atributo que são reservadas de Python
====
[source, python]
----
...
isso não aparece neste trecho de código.
...
usei duas propriedades em `Vector2d` apenas para que os atributos `x` e `y` fossem limitados a leitura (_read-only_).
...
O método `+Record.__init__+` ilustra um velho truque. Lembre-se que o
...
problema não ocorre conjunto de dados que estamos explorando.

====

A definição de `Record` no <<ex_schedule_v1>> é tão simples que você pode estar
se perguntando porque não a usei antes, em no lugar de `FrozenJSON`.
...
devolverá um lista de instâncias de `Record`. Esta colisão de nomes exige uma
...
condições de de corrida que podem levar à corrupção de dados.
...
_cache_ `+__speaker_objs+` e a segunda thread corre o risco de ler
um valor inconsistente do _cache_.

Felizmente, Python 3.8 introduziu o decorador `@functools.cached_property`, que
é seguro para uso com threads. Infelizmente, ele vem com algumas ressalvas,
...
atributo de mesmo nome. Leituras e escritas subsequentes daquele atributo tem
...
documentação de https://fpy.li/bg[@functools.cached_property]. Sei que autor
...
vimos que quando uma instância e sua classe tem um atributo de dados com o mesmo nome,
...
como documentação da propriedade toda. O <<help_foo_screens>> mostra telas de
...
não precise redigitar o nome do atributo é uma problema
...
`quantity`.footnote:[Este código foi adaptado da _Recipe 9.21. Avoiding
Repetitive Property Methods_ (Evitando Métodos Repetitivos de
Propriedades) do https://fpy.li/pycook3[_Python Cookbook, 3rd. ed._], de David
Beazley e Brian K. Jones (O’Reilly).]
...
Na prática, por serem invocados incondicionalmene, afetando praticamente todos
...
só lida acessos a atributos que não existem. Usar propriedades ou descritores
tende ser mais fácil que definir um destes dois métodos especiais.
...
propriedades—para fornecer um solução genérica elegante, usando para isso o
...
(Evitando Métodos demPropriedade Repetitivos),
...
dos panos. Assim, quando Martelli et al. chegam à propriedades, eles concentram
...
outras. Apenas na última página do livro o autor revela que a "notação"
extremamente legível usada como se fosse pseudocódigo ao longo livro é
...
A digressão anterior toca um ponto relevante para programação: apesar do
...
em programaçõa: elas dificultam raciocinar sobre o custo de computar
...
sintaxe que a invocação de uma função: `my_obj = foo()`,
onde `foo` pode ser uma classe ou qualquer
outro invocável.

Outras linguagens, influenciadas pela sintaxe do {cpp}, tem um operador `new`
que faz a instanciação parecer diferente de uma invocação. Na maior parte do
tempo, o usuário de uma API não se importa se `foo` é uma função ou uma classe.
Por anos tive a impressão que `property` era uma função. Para o uso normal, não
faz diferença.
...
(como vimos na <<flexible_new_sec>>), ou devolve instâncias pré-fabricadas em vez criar uma nova instância toda vez.
...
Por outro lado, dezenas de classes na biblioteca padrão tem nomes apenas em minúsculas
(por exemplo, `property`, `str`, `defaultdict`, etc.).
Daí que talvez o uso de nomes de classe apenas com minúsculas seja um vantagem, e não um bug.
...
Então eu, pessoalmente, uso `CamelCase` em nas classes que escrevo,
e gostaria que todas as classea na biblioteca padrão de Python seguissem alguma convenção.
Estou olhando para vocês, `collections.OrderedDict` e `collections.defaultdict`.((("", startref="DAPsoap22")))
****
