...
equivalente escrita com a ajuda do decorador `dataclass`, como mostra o <<coord_dataclass_ex>>.
...
[[ex_named_tuple_2]]
.Atributos e métodos das tuplas nomeadas (continuando do exemplo anterior)
====
[source, python]
----
>>> City._fields  <1>
...
como o https://fpy.li/mypy[Mypy] ou o checador de tipos embutido do
...
causa uma exceção se `eq=False`, ou se qualquer um dos métodos de comparação que seriam gerados estiver definido ou for herdado.
...
O <<hackerclub_ex>> funciona como esperado, mas não é satisfatório pra um checador de tipos estático.
...
https://fpy.li/5-14["Code Smell" (_Cheiro no Código_)].
...
sem vincular uma variável (o corpo do `case` pode referir-se a `x` diretamente, se necessário):
...
https://fpy.li/pep526[PEP 526—Syntax for Variable Annotations (_Sintaxe para Anotações de Variáveis_)] (EN).
O aspecto provavelmente mais surpreendente das dicas de tipo em geral
...
O site https://fpy.li/5-28[_Refactoring Guru_ (Guru da Refatoração)] também tem uma descrição do
https://fpy.li/5-29[data class code smell (_classe de dados como cheiro no código_)].
