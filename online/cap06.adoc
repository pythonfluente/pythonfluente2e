...
[[ex_a_b_refs]]
.As variáveis `a` e `b` referem-se à mesma lista, não cópias da lista.
====
[source, python]
----
>>> a = [1, 2, 3]  <1>
...
O `is` também é útil para testar valores de classes derivadas de `enum.Enum`.
Se não estiver seguro, use `==`. Em geral, é o que você quer,
e ele também funciona com `None` e valores de `Enum`,
ainda que seja um pouco mais lento.
====
...
(`bus3`). Então vemos o que acontece quando `bus1` deixa um passageiro.
...
<4> Surpresa: `a` e `b` se referem ao mesmo objeto `str`!
<1> Alguns inteiros pequenos são compartilhados.
<2> Outros inteiros não são compartilhados.
