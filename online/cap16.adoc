...
____
Algumas coisas me deixam meio dividido, como a sobrecarga de operadores. Deixei a sobrecarga de operadores de fora em uma decisão bastante pessoal, pois tinha visto gente demais abusar [deste recurso] no {cpp}.footnote:[Fonte: https://fpy.li/16-1[_The C Family of Languages: Interview with Dennis Ritchie, Bjarne Stroustrup, and James Gosling_] (A Família de Linguagens C: entrevista com Dennis Ritchie, Bjarne Stroustrup, e James Gosling).]
____
...
* O tratamento default de operadores de atribuição aumentada tal como `{iadd}`, e como sobrecarregá-los
...
Por muitos anos, o produto escalar (_dot product_) como `numpy.dot(a, b)`
na biblioteca NumPy.
...
3.5, então nós e os os desenvolvedores da NumPy podemos implementar o operador
...
`abc.Iterable`. Ambas ABCs implementam o `+__subclasshook__+`, portanto
...
<3> Um `Vector` também é considerado igual a uma `tuple` ou qualquer sequência
com itens escalares de valor igual.

O último resultado no <<eq_initial_demo>> pode ser indesejável. Queremos mesmo
que um `Vector` seja considerado igual a uma `tuple` contendo os mesmos números?
...
Então, seremos conservadores e faremos checagem de tiposs. Se o
...
Vamos chamar a subclasse de `AddableBingoCage`. O doctests da classe
...
estudar sua implementação no <<ex_addable_bingo>>. Lembre-se que `BingoCage`,
...
embutida `iter` será tratada no <<ch_generators>>. Eu poderia ter usado
`tuple(other)` aqui, e isso funcionaria, ao custo de criar uma nova `tuple`
pois o método `.load(…)` só precisa iterar sobre seu argumento.]
