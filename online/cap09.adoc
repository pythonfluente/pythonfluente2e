...
quando são invocadas explicitamente. Isso ressalta a diferença entre
_momento da importação_ e _momento da execução_ na operação de um módulo em Python.
...
reflete essa decisão, e tentará acessar `b` no escopo local. Mais tarde, quando a
...
cresce continuamente; por exemplo, o preço de diário de um produto
através de toda a sua história. A cada dia, um novo preço é acrescentado,
e a média é computada levando em conta todos os preços até ali.
...
[[ex_average_broken]]
.Um função de ordem superior incorreta para calcular um média contínua sem armazenar todo o histórico
====
[source, python]
----
def make_averager():
...
O <<ex_average_fixed>> é a implemetação correta da versão otimizada de `make_averager`.

[[ex_average_fixed]]
.Calcula uma média contínua sem armazenar todo o histórico
====
[source, python]
----
def make_averager():
...
* Se há uma declaração `nonlocal x`, então `x` está vinculdada à variável local `x` na função circundante mais próxima de onde `x` for definida.
...
Então `factorial` agora tem uma referência para a função `clocked`. Daqui por
...
lembre-se que `@` é açúcar sintático para aplicar a função decoradora à função
...
o _cache_ interno usa um `dict` para armazenar os resultados, e as chaves
...
demais, e o acoplamento entre ela e as funções especializadas seria forte demais.
...
argumento. É isso que signifca o termo _single dispatch_ (despacho único).
Se mais argumentos fossem usados para selecionar a função específica,
temos despacho múltiplo (_multiple dispatch_).
...
está disponível no PyPI, no pacote https://fpy.li/9-9[_decorator_].
...
do ambiente onde a funcão é invocada.
