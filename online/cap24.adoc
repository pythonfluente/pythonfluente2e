...
Em Python, classes são objetos de primeira classe, então uma função criar uma nova classe a qualquer momento, sem usar a palavra-chave `class`,
e sem manipular código-fonte ou bytecodes.

Decoradores de classes também são funções, mas são servem para inspecionar, modificar ou até substituir a classe decorada por outra classe.
...
A <<metaclases_real_world_sec>> é nova, trazendo algumas considerações de alto nível sobre a aplicabilidade das metaclasses.
...
`+__name__+`, and `+__mro__+`. Outros atributos de classe padrão são:
...
https://fpy.li/24-3[_PEP 3155—Qualified name for classes and functions] (Nome qualificado para classes e funções).
...
Usando o construtor `type`, podemos criar uma classe idêntica em tempo de execução, com o seguinte código:
...
`cls_dict`::
    ...
    Invocáveis se tornam métodos, como vimos na <<methods_are_descriptors_sec>>.
...
<<enhancing_with_init_subclass_sec>>, mas tratarei o uso de argumentos
...
Vamos criar metaclasses customizadas na <<metclass101_sec>>.
...
No <<ch_dataclass>> também vimos `typing.NamedTuple` e `@dataclass`.
...
que cria classes simples como `Dog` em tempo real. O <<record_factory_demo>> mostra como ela deve funcionar.
...
----
====
<1> A fábrica pode ser invocada como `namedtuple`: nome da classe, seguido dos nomes dos atributos separados por espaços, em um única string.
<2> Um `repr` agradável.
...
A última linha de `record_factory` no <<record_factory_ex>> cria uma classe cujo
...
No <<ch_dataclass>>, vimos como `typing.NamedTuple` e `@dataclass` usam a sintaxe da instrução `class` para especificar atributos para uma
...
Não é possível suportar toda dica de tipo estática concebível para checagem de tipos durante a execução.
Entretanto, algums tipos que são também classes concretas podem ser usados com `Checked`.
Isto inclui tipos simples, usados com frequência para o conteúdo de campos, como  `str`, `int`, `float`, e `bool`, bem como listas destes tipos.
...
----
====
<1> `Movie` herda de `Checked`—que definiremos mais tarde, no <<ex_checked_class_top>>.
...
[TIP]
====
Na <<auto_storage_sec>>, vimos o conveniente método especial `+__set_name__+` para descritores.
...
`+Checked.__init_subclass__+`, que veremos no <<ex_checked_class_top>>.
====

Vamos agora nos concentrar na classe `Checked`, que dividi em duas listagens. O <<ex_checked_class_top>> mostra a parte inicial da classe, incluindo os métodos mais importantes para este exemplo.
...
Reveja a <<problems_annot_runtime_sec>> para uma discussão dos problemas com
essas funções.
...
informações sobre isso, veja a seguir a caixa _<<init_subclass_not_typical_box>>_.
...
mesma árvore de herança. Veja a <<mult_inherit_mro_sec>>.
...
<<good_poison_pill_tip>>, o objeto `Ellipsis` é um valor sentinela conveniente e
...
<<checked_class_bottom_ex>>. Ele recebe um argumento `*names` com os nomes de
...
continuando do <<ex_checked_class_top>>.
...
Este é um raro exemplo do tipo especial `NoReturn`, tratado na <<noreturn_sec>>.
...
Vamos explorar essas questões de sincronia na <<import_v_runtime_sec>>.
...
Veremos como configurar `+__slots__+` dinamicamente na <<metclass101_sec>>.
...
Decoradores de classe frequentemente devolvem a própria classe decorada, após
...
porque ela foi implementada como um decorador de classes.
...
<1> Lembre-se que classes são instâncias de `type`. Estas dicas de tipo indicam
...
<<ex_checked_class_top>>. Aqui há mais trabalho a ser feito...
...
* As funções no <<checkeddeco_methods_ex>> serão injetadas na classe decorada,
...
Elas foram explicadas no <<ex_checked_class_top>> e no <<checked_class_bottom_ex>>.
...
declaraçãofootnote:[Compare com a instrução `import` em Java, que é apenas uma
declaração para informar o compilador que determinados pacotes são
necessários.], pois na verdade ela executa todo o código no nível superior de um
módulo, quando este é importado para o processo pela primeira vez. Importações
posteriores do mesmo módulo usarão um _cache_, e então o único efeito será a
vinculação dos objetos importados a nomes no módulo cliente. O código
executado em consequência de um `import` pode fazer qualquer coisa, incluindo ações típicas da "execução",
como escrever em um arquivo de log ou conectar-se a um banco de
dados.footnote:[Não estou dizendo que é uma boa ideia abrir uma conexão com um
banco de dados só porque o módulo foi importado, apenas apontando que isso pode
ser feito.] Por isso a fronteira entre a "importação" e a "execução" é difusa:
`import` pode acionar todo tipo de comportamento de "execução", porque a
instrução `import` e a função embutida `+__import__()+` podem ser usadas dentro
de qualquer função normal.

Tudo isso é bastante abstrato e sutil, então vamos realizar alguns experimentos para ver o que acontece, e quando.
...
https://fpy.li/2j["Modelo de Dados"] da referência de Python.
...
Uma metaclasse pode customizar suas instâncias implementando métodos especiais, como demosntram as próximas seções.((("", startref="MCbasics24")))
...
Para processar essa instrução `class`, Python invoca `+MetaKlass.__new__+` com os seguintes argumentos:
...
podemos também aplicar processamento adicional à classe recém-criada, antes de devolvê-la para Python. Python então invoca `+SuperKlass.__init_subclass__+`, passando a classe que criamos, e então aplicando um decorador de classe, se algum estiver presente.
Finalmente, Python vincula o objeto classe a seu nome no espaço de nomes circundante—normalmente o espaço de nomes global do módulo, se a instrução `class` foi uma instrução no primeiro nível.
...
`+__slots__+`, como discutido na <<why_cannot_config_slots_sec>>.
...
<3> `defaults` vai preservar um mapeamento de nomes de atributos e seus valores default.
...
`+__init__+`, `+__repr__+` e outros atributos definidos pelo Python, como `+__qualname__+` e
`+__module__+`.
...
O principal caso de uso para `+__prepare__+` antes de Python 3.6 era oferecer um
`OrderedDict` para guardar os atributos de uma classe em construção, para que o `+__new__+` da metaclasse pudesse processar aqueles atributos na ordem em que aparecem no código-fonte da definição de classe do usuário.
...
(listada no <<ex_checked_class_top>> e no <<checked_class_bottom_ex>>), com três modificações:
...
Python só invoca `+__prepare__+` em metaclasses e então, se fosse necessário processar o espaço de nomes da classe na ordem em que eles aparecem o código-fonte, antes de Python 3.6 era preciso usar uma metaclasse.
...
Nenhum dos acréscimos que mencionei na <<metaclass_modern_features_sec>> quebrou código existente que usava metaclasses. Mas código legado com metaclasses frequentemente pode ser simplificado através do uso daqueles recursos, especialmente se for possível ignorar versões de Python anteriores à 3.6—versões que não são mais mantidas.
...
Quando Python processa o espaço de nomes da classe do usuário e lê `banana`, ele procura aquele nome no mapeamento fornecido por `+__prepare__+`: uma instância de `WilyDict`.
...
Python se contenta com isso, e daí tenta recuperar `'coconut'`.
...
Enquanto experimentava, descobri que Python procurava `+__name__+` no espaço de nomes da classe em construção, fazendo com que `WilyDict` acrescentasse um item `+__name__+` e incrementasse `+__next_value+`.
Eu então inseri uma instrução `if` em `+__missing__+`, para gerar um `KeyError` para chaves que se parecem com atributos _dunder_.

O pacote _autoconst.py_ tanto exige quanto ilustra o mecanismo de criação dinâmica de classes de Python.
...
[NOTE]
====
O método especial `+__class_getitem__+` foi introduzido no Python 3.9 para suportar tipos genéricos, como parte da https://fpy.li/pep585[PEP 585—Type Hinting Generics In Standard Collections (_Dicas de Tipos Genéricas em Coleções Padrão_)].
Graças a `+__class_getitem__+`, os mantenedores de Python não precisaram escrever uma nova metaclasse para que os tipos embutidos implementassem `+__getitem__+`, de modo que fosse possível escrever dicas de tipo genéricas, tal como `list[int]`.
...
https://fpy.li/24-17[traits (_traços_)] e
...
[quote, David Heinemeier Hansson, criador de Ruby on Rails]
____
Bons frameworks são extraídos, não inventados.footnote:[Essa frase é muito citada. Encontrei uma citação direta antiga em um https://fpy.li/24-19[post] de 2005 no blog de DHH.]
____
...
`+__new__+` de uma metaclasse são invocados mais cedo que `+__init_subclass__+` e decoradores de classe, oferecendo oportunidades para uma customização de classes mais profunda.

A terceira versão de uma fábrica de classes `Checked`, com descritores `Field` e uma configuração customizada de `+__slots__+` foi apresentada, seguida de considerações gerais sobre o uso de metaclasses na prática.
...
Nunca encontrei outra linguagem como Python, fácil para iniciantes, prática para profissionais e empolgante para hackers.
...
As referências essenciais deste capítulo na documentação de Python são https://fpy.li/by["3.3.3. Personalizando a criação de classe"] no capítulo "Modelos de Dados" da _Referência da Linguagem Python_, que cobre `+__init_subclass__+` e metaclasses. A
https://fpy.li/c4[documentação da classe `type`]
na página "Funções Embutidas", e
https://fpy.li/bt["4.13. Atributos especiais"] do capítulo "Tipos embutidos" na _Biblioteca Padrão de Python_ também são leituras fundamentais.

Na _Biblioteca Padrão de Python_, a https://fpy.li/bz[documentação do módulo `types`] trata de duas funções introduzidas no Python 3.3, que simplificam a metaprogramação de classes: `types.new_class` and `types.prepare_class`.

Decoradores de classes foram formalizados na https://fpy.li/24-25[PEP 3129—Class Decorators (_Decoradores de Classes_)], escrita por Collin Winter, com a implementação de referência desenvolvida por Jack Diederich. A palestra "Class Decorators: Radically Simple"  (_Decoradores de Classes: Radicalmente Simples_. Aqui o https://fpy.li/24-26[video]), na PyCon 2009, também de Jack Diederich, é uma rápida introdução a esse recurso.
Além de `@dataclass`, um exemplo interessante—e mais simples—de decorador de classes na biblioteca padrão de Python é https://fpy.li/7q[`functools.total_ordering`], que gera métodos especiais para comparação de objetos.
...
O pass:[<a href="https://fpy.li/pynut3" class="orm:hideurl"><em>Python in a Nutshell</em></a>], 3ª ed., de Alex Martelli, Anna Ravenscroft, e Steve Holden, é uma referência, mas foi escrito antes da
https://fpy.li/pep487[_PEP 487_] simplificar o processo de customizar classes na sua criação.
O principal exemplo de metaclasse no livro—`MetaBunch`—ainda é válido, pois não pode ser escrito com mecanismos mais simples.
...
https://fpy.li/24-28["Unifying types and classes in Python 2.2" (_Unificando tipos e classes no Python 2.2_)].
...
Se você gosta de metaprogramação, talvez gostaria que Python suportasse o recurso definitivo de metaprogramação: macros sintáticas, como as oferecidas pela família de linguagens Lisp e—mais recentemente—pelo Elixir e pelo Rust.
...
https://fpy.li/pep638[PEP 638—Syntactic Macros (_Macros Sintáticas_)], defendendo exatamente isso.
...
. *A visão radical*: Programas de computador se tornaram muito grandes e complexos para serem apreendidos pela mente humana. Portanto, a tarefa da educação na ciência da computação é ensinar os estudantes como expandir suas mentes até que os programas caibam ali, aprendendo a pensar com um vocabulário de ideias maiores, mais poderosas e mais flexíveis que aquelas óbvias. Cada unidade de pensamento programático deve gerar uma grande recompensa para as capacidades do programa.footnote:[Brian Harvey e Matthew Wright, _Simply Scheme_ (MIT Press, 1999), p. xvii. O texto completo está disponível em https://fpy.li/24-30[Berkeley.edu].]
____
...
Nesse ponto você já deve ter adivinhado que concordo com a visão "radical",
...
praticamente exigida em Java e suportada pela geração de _getters/setters_ através de atalhos do teclado por IDEs Java.
...
Esse framework é tão eficiente que mecanismos essenciais de Python o utilizam por baixo dos panos.
...
É uma façanha marcante do projeto, que uma linguagem acessível para um iniciante forneça abstrações poderosas, como fábricas de classe, decoradores de classe, e metaclasses completas e definidas pelo usuário.
Melhor ainda, os recursos avançados estão integrados de forma a não atrapalhar a programação casual (eles na verdade ajudam nisso, por trás da cortina).
...
Os melhores recursos da linguagem são aqueles que beneficiam a todos, mesmo que alguns usuários de Python não os conheçam. Mas esses usuários sempre podem aprender, e criar a próxima grande biblioteca.

Espero notícias sobre suas contribuições ao ecossistema e à comunidade de Python!
****
