...
que cria classes simples como `Dog` dinamicamente. O <<record_factory_demo>> mostra como ela deve funcionar.
...
Em Python, `import` serve para isso, mas também carrega executa todo o código no
...
Note que as linhas exibidas por _builderlib.py_ tem uma `@` à esquerda.
...
Note que a função `main()` de _evaldemo.py_ (<<evaldemo_ex>>)
rodou durante a sessão no console (<<evaldemo_console_ex>>), portanto nenhuma instância de `Klass` foi criada.
Todas as ações que vimos foram acionadas por operações no momento da importação:
importar `builderlib` e definir `Klass`.

Se você executar _evaldemo.py_ como um script, vai ver a mesma saída do <<evaldemo_console_ex>>,
com mais linhas logo antes do final.
As linhas adicionais são o resultado da execução de `main()` no <<evaldemo_script_ex>>:
...
deveria se preocupar com elas. Quem se pergunta se precisa delas, não precisa
...
A <<meta_class_and_class_mgn>> usa a _Notação de Engenhocas e Bugigangas_ para ilustrar uma metaclasse:
...
As classes `object` e `type` tem uma relação singular: `object` é uma instância
...
Isto foi discutido na <<flexible_new_sec>>.
...
é uma variação da <<evaldemo_sec>>, acrescentando uma metaclasse para ter mais emoção.
O módulo _builderlib.py_ é o mesmo de antes, mas o script principal agora é _evaldemo_meta.py_,
listado no <<evaldemo_meta_ex>>.
...
[WARNING]
====
Em nome da ciência, o <<evaldemo_meta_ex>> desafia qualquer razão prática e aplica três técnicas diferentes de metaprogramação em `Klass`:
...
Se executarmos _evaldemo_meta.py_ como um script, `main()` é invocado, e mais coisas acontecem. Veja o <<evaldemo_meta_script_ex>>.
...
então aqui temos um cenário de ficção para justificar a reescrever _checkedlib.py_ com `+__slots__+`, exigindo a aplicação de uma metaclasse.
...
Vamos agora estudar o código, começando pela classe `Field` do <<checked_field_meta_ex>>.
...
O <<checked_metaclass_ex>> mostra o código da metaclasse que controla este exemplo.
...
sendo a forma de especificar a metaclasse a ser usada, algo que no Python 3 é
feito esta sintaxe:
...
Nenhum dos acréscimos que mencionei na <<metaclass_modern_features_sec>> quebrou
...
id="prepare24"))) atualizei esse capítulo para a segunda edição, precisei
encontrar exemplos simples mas reveladores, para substituir o código de
`LineItem` no exemplo da loja de comida a granel, que não exigir mais
...
`if` em `+__missing__+`, para gerar um `KeyError` quando uma chave se parecem com
...
* Implemente uma nova variante de `Enum`. Isso é um empreeendimento
...
tipo genéricas, tal como `list[int]`. Esse é um recurso restrito, mas
representativo, de um caso de uso mais amplo para metaclasses: implementar
operadores e outros métodos especiais para funcionarem a nível de classes, tal
...
____
Bons frameworks são extraídos, não inventados.footnote:[Esta frase é muito citada. Encontrei uma citação direta antiga em um https://fpy.li/24-19[post] de 2005 no blog de DHH.]
____
...
https://fpy.li/24-25[PEP 3129—Class Decorators]
...
Python moderno, pois cobre o chamado de "novo estilo" de
semântica de classes—ao comportamento padrão no Python 3—incluindo descritores e
...
programa que atende as especificações.
...
Aguardo notícias sobre suas contribuições ao ecossistema e à comunidade de Python!
****
