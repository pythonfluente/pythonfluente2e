...
|          `+d.__ror__(other)+` |  ●   |      ●      |      ●      | Suporte a `other \| dd`—operador de união invertido (Python ≥ 3.9)footnote:[Operadores reversos são tratados no <<ch_op_overload>>.]
...
|   S ∩ Z   | `s & z`  |                        `+s.__and__(z)+` | interseção de `s` e `z`
...
|   S ∆ Z   | `s ^ z`  |                        `+s.__xor__(z)+` | Diferença simétrica (o complemento da interseção `s & z`)
