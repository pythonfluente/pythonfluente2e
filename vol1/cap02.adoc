...
O motivo será explicado em <<what_is_hashable_sec>>.
...
«_Are tuples more efficient than lists in Python?_ (As tuplas são mais eficientes que as listas no Python?)» [.small]#&#91;fpy.li/2-3&#93;#
no StackOverflow.
...
extirpadas e, de várias maneiras modificadas internamente com o uso da notação de fatias
...
Quando vi isso, tinha certeza que a resposta era **B**, mas, na verdade é **C**:
...
método `list.sort` ordena uma lista internamente—isto é, sem criar uma cópia.
...
embaralhar os itens de uma sequência mutável internmante (_in-place_),
...
tipo `list` é flexível e fácil de usar mas, dependendo do caso, há opções melhores.
...
«_When should a memoryview be used?_ (Quando se deve usar uma memoryview?)» [.small]#&#91;fpy.li/2-17&#93;#:
...
|           `s.remove(e)` |   ●   |       | Remove o primeiro item de valor igual a `e`
|           `s.reverse()` |   ●   |   ●   | Inverte a ordem do itens internamente
|    `+s.__reversed__()+` |   ●   |   ●   | Obtém iterador para percorrer itens, do último para o primeiro
...
«_PEP 636—Structural Pattern Matching: Tutorial_ (Casamento de Padrão Estrutural: Tutorial_)» [.small]#&#91;fpy.li/pep636&#93;#
...
«documentação do "Modelo de Dados"» [.small]#&#91;fpy.li/2x&#93;#:
