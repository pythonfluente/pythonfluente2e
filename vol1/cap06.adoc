...
Python#x2014;lists, dicts, sets, etc..&#x2014;são contêiners:
...
Agora, no <<ex_bus1_console>> interativo, vamos criar um objeto `bus` e
...
Para demonstrar, no <<ex_haunted_bus>>,
...
Se duas variáveis se referem a objetos imutáveis de igual valor (quando `a == b` é `True`),
