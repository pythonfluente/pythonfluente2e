...
a instrução `del` e de algumas otimizações com objetos imutáveis em Python.
...
O módulo `copy` oferece as funções `deepcopy` e `copy`que,
respectivamente, retornam cópias profundas e rasas de objetos arbitrários.
...
[[ex_cycle1]]
.Referências cíclicas: `b` tem uma referência para `a` e em seguida é concatenado com `a`; ainda assim, `deepcopy` consegue copiar `a`.
====
[source, python]
----
>>> a = [10, 20]
...
Agora vamos conversar sobre uma das instruções mais mal compreendidas em Python:
...
Leia «"Weak References" em _fluentpython.com_» [.small]#&#91;fpy.li/weakref&#93;#
