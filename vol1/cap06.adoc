...
[[ex_a_b_refs]]
.As variáveis `a` e `b` mantêm referências para a mesma lista, não cópias da lista.
====
[source, python]
----
>>> a = [1, 2, 3]  <1>
...
Para ilustrar o porquê, no <<ex_haunted_bus>>,
...
[[ex_finalize]]
.Assistindo ao fim de um objeto quando não resta nenhuma referência apontando para ele
...
Se duas variáveis se referem a objetos imutáveis de igual valor (`a == b` é `True`),
