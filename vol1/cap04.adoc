...
Observe que as últimas três codificações usam múltplos bytes e tamanho variável.
...
Por exemplo, a palavra mojibake em japonês pode aparecer como quatro tofus: 文字化け.
Aqui, o motivo é que o programa `asciidoctor-pdf` que usei para gerar o livro impresso
...
uma vez que você supõe que uma série de bytes é um((("plain text"))) texto puro,
...
são dificultadas pelo fato do Unicode permitir combinações de caracteres:
...
Um passo ainda mais radical substituiria os símbolos comuns em textos de línguas européias
...
<<<
[[ex_asciize]]
.Transforma alguns símbolos tipográficos européias em ASCII (este trecho também é parte do simplify.py do <<ex_shave_marks>>)
====
[source, python]
----
...
«_What's New in Python 3.0_ (O quê há de novo no Python 3.0)» [.small]#&#91;fpy.li/4-36&#93;#,
...
(Receita 5.17. Escrevendo Bytes em um Arquivo de Texto_),
mostrando que qualquer arquivo de texto há sempre uma sequência de bytes
...
(Receita 6.11. Lendo e Escrevendo Arrays de Registros Binários).
...
"texto puro" (_plain text_) não significa "ASCII". O «_Unicode Glossary_» [.small]#&#91;fpy.li/4-55&#93;#
...
muito bem—incluindo os caracteres kanji da palavra "mojibake": 文字化け.
****
...
Entretanto, sempre podemos contar com pythonista Armin Ronacher para criticar o Python 3.
...
a facilitade de buscar um caractere arbitrário pela posição é
