...
[[ex_named_tuple_2]]
.Atributos e métodos das tuplas nomeadas (continuando o exenplo anterior)
====
[source, python]
----
>>> City._fields  <1>
...
Lembre como criamos a classe `Card` no <<ex_pythonic_deck>> do <<ch_data_model>>:
...
Primeiro, veja comportamento esperado de uma subclasse de `ClubMember`,
...
O <<hackerclub_ex>> funciona como esperado, mas não é satisfatório pra um checador estático de tipos.
...
Para deixar felizes tanto o checador de tipos quando o `@dataclass`,
...
Abaixo está a tradução integral daquele artigo.footnote:[Tive a felicidade de ter
Martin Fowler como colega de trabalho na Thoughtworks,
estão esperei apenas 20 minutos para receber sua permissão.]
...
Mencionei "primeiro" e o "terceiro" atributo, mas o quê isso realmente significa?
...
é sobre Guido van Rossum. Entre outras coisa, ele diz:
...
Você precisa recorrer a aquela anotação usando o pseudo-tipo `ClassVar`,
que é uma gambiarra:
