...
Mais tarde no <<ch_data_model>>, escrevi uma função `spades_high`, para ordenação.
...
Considero essa limitação de Python é uma benção.]
...
Para fazer felizes tanto o checador de tipos quanto o `@dataclass`,
...
Os melhores cheiros são algo fácil de detectar e que, na maioria das vezes, levam a problemas realmente interessantes.
