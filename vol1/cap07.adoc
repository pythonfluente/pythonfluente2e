...
Ambos serão estudados no Capítulo 21 [vol.3, fpy.li/57], mas são mencionados aqui ao lado dos outros invocáveis.

A <<positional_only_params>> é nova, e fala de um recurso que surgiu no Python 3.8: parâmetros somente posicionais.

Transferi a discussão sobre acesso a anotações de funções durante a execução para a Seção 15.5 [vol.2, fpy.li/58].
...
Dou um explicação mais completa sobre `reduce` na Seção 12.7 [vol.2, fpy.li/59],
onde um exemplo mais longo, atravessando várias seções, cria um contexto
significativo para o uso dessa função.
As funções de redução serão resumidas mais à frente no livro, na Seção 17.10 [vol.3, fpy.li/5a],
...
Ambos são tratados no Capítulo 17 [vol.3, fpy.li/4s].
...
Elas são o assunto do Capítulo 21 [vol.3, fpy.li/57].
...
Clausuras e decoradores são o assunto do Capítulo 9 [vol.2, fpy.li/5c].
...
Essas funções são tratadas no Capítulo 9 [vol.2, fpy.li/5c], que também explica como implementar decoradores customizados.((("", startref="FAFfp07")))((("", startref="fprogpack07")))((("", startref="functools07")))
...
O Capítulo 9 [vol.2, fpy.li/5c] mergulha nos decoradores de função—um tipo especial de função de ordem superior—e no mecanismo de clausura (_closure_) que os faz funcionar.
O Capítulo 10 [vol.2, fpy.li/5d] mostra como as funções de primeira classe podem simplificar alguns padrões clássicos de projetos (_design patterns_) orientados a objetos.
...
é um excelente complemento a esse capítulo, bem como ao  Capítulo 9 [vol.2, fpy.li/5c],
...
O principal foco daquele texto, entretanto, é o uso de iteradores e geradoras, assunto do Capítulo 17 [vol.3, fpy.li/4s].
...
Prometo escrever mais sobre programação assíncrona no futuro, mas esse assunto será adiado até o Capítulo 21 [vol.3, fpy.li/57].
