...
programamos para explorar dados e ideias, como é o caso em ciência de dados, computação criativa e para aprender a programar.
...
Vamos falar disso mais adiante nesse capítulo, em <<protocols_in_fn_sec>>, e com mais detalhes no
«Capítulo 13» [.small]#&#91;vol.2, fpy.li/13&#93;.&#93;#]
...
O checador de tipos não executa qualquer pedaço do programa, ele apenas analisa o código-fonte.
...
e o checador de tipos analisa se aqueles métodos estão implementados onde um tipo daquele protocolo é exigido.
...
é conhecido como duck typing estático (_static duck typing_):
...
Isso significa que todos aqueles argumentos precisa ser do tipo `str`.
...
de Claudio Jolowicz, é uma introdução mas curta que também fala de validação de checagem de tipo durante a execução.
...
Me inquieta que a adoção de dicas de tipo em bibliotecas desencorajem programadores
de oferecer funções assim no futuro.
