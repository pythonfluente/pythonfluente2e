...
O Capítulo 15 [vol.2, fpy.li/54] explora as dicas de tipo no contexto de classes e outros recursos do módulo `typing`.
...
A utilidade dessa função será explicada na Seção 18.3.3 [vol.3, fpy.li/5e].
...
<<param_generics_typevar_sec>>—ou sobrecarga (_overloading_), que veremos na Seção 15.2 [vol.2, fpy.li/5f].
====
...
mais adequada para código server-side, que estudaremos no Capítulo 21 [vol.3, fpy.li/57].
...
----
====
<1> `tokenize` é uma função geradora, assunto do Capítulo 17 [vol.3, fpy.li/4s].
...
Isso é tratado na Seção 15.3 [vol.2, fpy.li/29], sobre `typing.TypedDict`.
====
...
O conceito de subclasse virtual será explicado no Capítulo 13 [vol.2, fpy.li/4q].
...
Voltaremps a essa questão na Seção 13.6.8 [vol.2, fpy.li/5g], que é dedicada a comparar protocolos e ABCs
...
. Se você quiser evitar a codificação explícita de tipos concretos, usar protocolos numéricos como `SupportsFloat`, tratados na Seção 13.6.2 [vol.2, fpy.li/5h].
...
Voltaremos a ele em Capítulo 17 [vol.3, fpy.li/4s], que trata de geradores e
...
que envolvem conceitos avançados que vamos deixar para a Seção 15.7 [vol.2, fpy.li/5j].
...
Os dois tipos de protocolo são tratados no Capítulo 13 [vol.2, fpy.li/4q].
...
Vamos voltar a ele na Parte IV, onde o Capítulo 13 [vol.2, fpy.li/4q] compara as abordagens da tipagem estrutural,
do duck typing e das ABCs—outro modo de formalizar protocolos.
Além disso, a Seção 15.2 [vol.2, fpy.li/5f] explica como
...
A solução está na Seção 13.6.1 [vol.2, fpy.li/5m].((("", startref="GTSstatic08")))((("", startref="stprot08")))((("", startref="proto08")))
====
...
A Seção 15.7 [vol.2, fpy.li/5j] explica variância em mais detalhes e
...
Elas serão também o ponto central do Capítulo 15 [vol.2, fpy.li/54], que trata de classes genéricas,
...
Como já mencionado, o Capítulo 15 [vol.2, fpy.li/54] cobre outros tópicos sobre tipagem, e a
Seção 15.10 [vol.2, fpy.li/5p] traz referências adicionais,
...
Porém, vou mostrar na Seção 15.2.1 [vol.2, fpy.li/5q] que são necessárias
