...
Atualizei e expandi «post de 18 páginas» [.small]#&#91;fpy.li/hashint&#93;# com diagramas e explicações sobre:
...
A «PEP 448—Additional Unpacking Generalizations (_Generalizações de Desempacotamento Adicionais_)» [.small]#&#91;fpy.li/pep448&#93;#
...
«"Motivation" (_Motivação_)» [.small]#&#91;fpy.li/3-1&#93;# (EN) da
«PEP 584—Add Union Operators To dict (_Acrescentar Operadores de União a dict_)» [.small]#&#91;fpy.li/pep584&#93;#
...
(os valores não precisam ser_hashable_, só as chaves).
Se você precisa de uma recapitulação, a próxima seção explica isso.

[[what_is_hashable]]
==== O que é hashable?

Aqui((("hashable, definition of"))) está parte da definição de_hashable_, adaptada do
«Glossário de Python» [.small]#&#91;fpy.li/32&#93;#:

[quote]
____
Um objeto é_hashable_ se tem um código de hash que nunca muda durante seu ciclo de vida
(precisa ter um método `+__hash__+`) e pode ser comparado com outros objetos
(precisa ter um método `+__eq__+`).
Objetos_hashable_ que são comparados como iguais devem ter o mesmo código de hash.footnote:[O
...
Tipos numéricos((("numeric types", "hashability of"))) e os tipos planos imutáveis `str` e `bytes` são todos_hashable_.
Tipos contêineres são_hashable_ se forem imutáveis e se todos os objetos por eles contidos forem também_hashable_.
Um `frozenset` é sempre_hashable_, pois todos os elementos que ele contém devem ser, por definição,_hashable_.
Uma `tuple` é_hashable_ apenas se todos os seus itens também forem.
...
ele será_hashable_ apenas se seu `+__hash__()+` sempre devolver o mesmo código de hash.
...
[options="header", cols="7,^2,^3,^4,15"]
|===========================================================================================================================================================================================================================================================================
...
Isso pode ser usado para contar instâncias de objetos_hashable_ ou como um _multiset_ (conjunto múltiplo),
...
[WARNING]
====
O `pickle` de Python é fácil de usar nos caso mais simples, mas há vários inconvenientes.
Leia o «Pickle’s nine flaws» [.small]#&#91;fpy.li/3-13&#93;#, de  Ned Batchelder,
...
(consulte o «código-fonte de Python» [.small]#&#91;fpy.li/3-14&#93;#).
...
A PEP 455 foi rejeitada em maio de 2015—veja a «mensagem de rejeição» [.small]#&#91;fpy.li/3-15&#93;# (EN)
de Raymond Hettinger.
Para experimentar com a `TransformDict`,
extraí o patch de Pitrou do «issue18986» [.small]#&#91;fpy.li/3-16&#93;# (EN)
para um módulo independente («_03-dict-set/transformdict.py_» [.small]#&#91;fpy.li/3-17&#93;#
...
* Chaves((("keys", "practical consequences of using dict"))) devem ser objetos_hashable_.
...
como explicado na Seção 11.11 [.small]#&#91;vol.2, fpy.li/52&#93;.&#93;#
Desde a implementação da
«PEP 412—Key-Sharing Dictionary (_Dicionário de Compartilhamento de Chaves_)» [.small]#&#91;fpy.li/pep412&#93;# (EN),
...
Para saber mais, leia «Internals of sets and dicts» [.small]#&#91;fpy.li/hashint&#93;# (EN).
...
O tipo `set` não é_hashable_, então não é possível criar um `set` com instâncias aninhadas de `set`.
Mas `frozenset` é_hashable_, então você pode ter instâncias de `frozenset` dentro de um `set`.
...
* Elementos de conjuntos precisam ser objetos_hashable_.
...
Veja o post «Internals of sets and dicts» [.small]#&#91;fpy.li/hashint&#93;# (EN)
...
mas `b`, `c`, e `d` podem ser iteráveis de qualquer tipo que produza itens_hashable_.
...
|====
|operador| método                           | 
| `s & z`  |                        `+s.__and__(z)+` | Intersecção de `s` e `z`
...
[WARNING]
====
Uma view obtida de `dict_items` só funciona como um conjunto se todos os valores naquele `dict` são_hashable_.
...
pois todas as chaves são_hashable_—por definição.
====
...
A «PEP 3106--Revamping dict.keys(), .values() and .items() (_Renovando dict.keys(), .values() e .items()_)» [.small]#&#91;fpy.li/pep3106&#93;#
...
O «PyPy» [.small]#&#91;fpy.li/3-19&#93;# foi o primeiro interpretador Python a implementar a proposta de Raymond Hettinger para dicts compactos,
e eles escreverem em seu blog sobre isso, em
«"Faster, more memory efficient and more ordered dictionaries on PyPy" (_Dicionários mais rápidos, mais eficientes em termos de memória e mais ordenados no PyPy_)» [.small]#&#91;fpy.li/3-20&#93;#
(EN), reconhecendo que um layout similar foi adotado no PHP 7,
como descrito em «PHP's new hashtable implementation (_A nova implementação de tabelas de hash de PHP_)» [.small]#&#91;fpy.li/3-21&#93;#
(EN). É sempre muito bom quando criadores citam trabalhos anteriores de outros.

Na PyCon 2017, Brandon Rhodes apresentou
«"The Dictionary Even Mightier" (_O dicionário, ainda mais poderoso_)» [.small]#&#91;fpy.li/3-22&#93;#
(EN), uma continuação de sua apresentação animada clássica
«"The Mighty Dictionary" (_O poderoso dicionário_)» [.small]#&#91;fpy.li/3-23&#93;#
...
«"Modern Dictionaries" (_Dicionários modernos_)» [.small]#&#91;fpy.li/3-24&#93;# (EN) de Raymond Hettinger,
...
«_Objects/dictobject.c_» [.small]#&#91;fpy.li/3-26&#93;# (EN) e no documento de design em
«_Objects/dictnotes.txt_» [.small]#&#91;fpy.li/3-27&#93;# (EN).

A justificativa para a adição de conjuntos ao Python está documentada na
«PEP 218--Adding a Built-In Set Object Type (_Adicionando um objeto embutido de tipo conjunto_)» [.small]#&#91;fpy.li/pep218&#93;#.
...
«"Set Practice: learning from Python's set types" (_A Prática dos Conjuntos: aprendendo com os tipos conjunto de Python_)» [.small]#&#91;fpy.li/3-29&#93;# (EN), («video» [.small]#&#91;fpy.li/3-28&#93;#),
...
«_The Go Programming Language_ (A Linguagem de Programação Go)» [.small]#&#91;fpy.li/38&#93;# (EN),
...
tabelas de hash!
Veja «"Hans Peter Luhn and the Birth of the Hashing Algorithm" (_Hans Peter Luhn e o Nascimento do Algoritmo de Hash_)» [.small]#&#91;fpy.li/3-31&#93;#.
...
JSON foi proposto como «"The Fat-Free Alternative to XML" (_A alternativa sem gordura ao XML_)» [.small]#&#91;fpy.li/3-32&#93;#
