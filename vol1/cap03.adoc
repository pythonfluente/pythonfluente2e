...
====
[role="pagebreak-before less_space"]
<1> É compatível com qualquer mapeamento na forma `'type': 'book', 'api' :2`, e uma chave `'authors'`
mapeada para uma sequência.
Devolve os itens da sequência, como uma nova `list`.
<2> É compatível com qualquer mapeamento na forma `'type': 'book', 'api' :1`, e uma chave `'author'`
...
<4> É compatível com qualquer mapeamento na forma `'type': 'movie'` e uma chave `'director'`
...
Se você tiver curiosidade, importe a função `dis` do módulo `dis`,
