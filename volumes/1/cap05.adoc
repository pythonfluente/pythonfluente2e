...
_Problemas com anotações durante a execução (v2:c15 fpy.li/2a)_.
...
Leia mais em
_Suporte a tipos de coleção descontinuados (v2:c8 fpy.li/2b)_.
...
Em
_Suportando o pattern matching posicional (v2:c11 fpy.li/2c)_
vamos estudar código para definir `+__match_args__+` em uma classe que criaremos
sem a ajuda de uma fábrica de classes.
...
`+__match_args__+`.
Por isso, às vezes pode ser necessário usar argumentos nomeados em um padrão, além dos argumentos posicionais.
====
