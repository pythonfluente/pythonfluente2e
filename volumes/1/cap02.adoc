...
E há um exemplo mais longo na fpy.li/2p?pattern_matching_case_study_sec[Pattern matching no lis.py: um estudo de caso] (vol. 4, cap. 18).
====
...
Nosso foco aqui é a desestruturação de sequências, então não vou explicar as ações do avaliador. Veja a fpy.li/2p?pattern_matching_case_study_sec[Pattern matching no lis.py: um estudo de caso] (vol. 4, cap. 18) para aprender mais sobre o funcionamento do _lis.py_.
...
[NOTE]
====
Veremos mais do _lis.py_ na fpy.li/2p?pattern_matching_case_study_sec[Pattern matching no lis.py: um estudo de caso] (vol. 4, cap. 18),
...
Como veremos na fpy.li/2p?how_slicing_works[Como funciona o fatiamento] (vol. 3, cap. 12), para avaliar a expressão `seq[start:stop:step]`,
o Python chama `+seq.__getitem__(slice(start, stop, step))+`.
Mesmo se você não for implementar seus próprios tipos de sequência,
é util saber que existem objetos `slice`, porque eles permitem que você atribua nomes às fatias,
...
Voltaremos aos objetos +slice+ quando formos discutir a criação de suas próprias coleções, na fpy.li/2p?sliceable_sequence[Vector versão #2: Uma sequência fatiável] (vol. 3, cap. 12).
