...
Esta edição impressa de _Python Fluente, Segunda Edição_ está dividida em cinco volumes,
correspondendo às cinco partes do original _Fluent Python, Second Edition_ (O'Reilly, 2022),
que foi publicado em um volume de 1012 páginas.
O texto completo dos cinco volumes desta edição em português está disponível em
...
mas muitas vezes assumo que você leu os capítulos precedentes de cada volume específico.

Tentei enfatizar o uso de classes e módulos que já existem antes de discutir como criar seus próprios.
Por exemplo, neste <<vl_data_str>>,
...
Criar sequências definidas pelo usuário só é discutido no _Volume III: Classes e Protocolos_,
...
Aqui estão os principais tópicos de cada volume:

{vl_data_str} (capítulos 1-6)::
