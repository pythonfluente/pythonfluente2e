...
isso no https://fpy.li/8x[«Exemplo 1 do Capítulo 1»] (vol.1), reproduzido aqui no <<ex_pythonic_deck_rep>>.

[[ex_pythonic_deck_rep]]
.Código do https://fpy.li/8x[«Exemplo 1 do Capítulo 1»] (vol.1), reproduzido aqui por conveniência
====
[source, python]
----
...
Falaremos mais sobre `super` na <<mro_section>>.
...
fizemos no <<hashable_vector2d>>, produz a mensagem `"read-only attribute"`
...
Entretanto, por todas as ressalvas discutidas na <<problems_with_slots>>, usar
