...
isso no https://fpy.li/8x[«Exemplo 1 do Capítulo 1»] (vol.1), reproduzido no <<ex_pythonic_deck_rep>>.
...
um dos argumentos é consumido. Veja a caixa <<zip_box>> logo adiante para saber mais sobre zip.
====

O <<ex_eq_loop>> é eficiente, mas a função `all` pode produzir o mesmo resultado do laço `for` em uma linha:
...
Confira o <<ex_eq_all>>: novamente, comparamos os `len` para não invocar `zip`
se os vetores têm tamanhos diferentes.((("", startref="eq12")))((("", startref="hash12")))((("",
startref="SSMhasheq12")))((("", startref="VCMhasheq12")))

[[ex_eq_all]]
.`+Vector.__eq__+` com `zip` e `all`: mesma lógica do <<ex_eq_loop>>
====
[source, python]
----
    def __eq__(self, other):
