...
implementação. O <<sum_overload_ex>> mostra como `sum` apareceria anotada e
...
pois os tipos de mapeamento que vimos na <<mapping_type_sec>> limitam os valores
a um mesmo tipo.
...
fixo de chaves_) resolve este problema. O <<bookdict_ex>> mostra um
`TypedDict` simples.
...
listadas na <<builders_compared_tbl>> (https://fpy.li/5[«Capítulo 5»] (vol.1)).
...
<<ex_checked_class_top>>, classe que estudaremos no https://fpy.li/24[«Capítulo 24»] (vol.3):
...
Já vimos o conceito de _variância_ na <<callable_variance_sec>>, aplicado a
...
<<callable_variance_sec>>. Além disso, https://fpy.li/15-32[`Generator`],
...
"produzir" são definidos na <<classic_coroutines_sec>>.
