...
A classe `Vector2d` do <<ex_vector2d_v0_demo>> é implementada em _vector2d_v0.py_ (no
<<ex_vector2d_v0>>). O código está baseado no https://fpy.li/8w[«Exemplo 2 do Capítulo 1»] (vol.1), exceto pelos
métodos para os operadores `{plus}` e `*`, que veremos mais tarde no
<<ch_op_overload>>. Vamos acrescentar o método para `==`, pois ele facilita
...
No <<vector_match_keyword_ex>>, os padrões nomeados funcionam como esperado.
...
e "como o usuário quer vê-lo" para a `str()`, na <<object_repr_sec>>.
