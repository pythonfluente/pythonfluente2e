...
O <<ex_vector_add_t1>> permite somar um `Vector` a um `Vector2d` ou
uma tupla:
...
.O <<ex_vector_add_t1>> falha quando o operando à esquerda não é `Vector`
====
[source, python]
----
>>> v1 = Vector([3, 4, 5])
...
<1> Nenhuma mudança no `+__add__+` do <<ex_vector_add_t1>>; repeti o código aqui
...
ou até uma instância de `fractions.Fraction`. No <<ex_vector_v7>>, o método
