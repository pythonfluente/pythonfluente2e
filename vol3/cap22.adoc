...
Usei o console de Python para explorar os dados interativamente, como mostra o <<ex_osconfeed_explore>>.
...
O <<ex_explore0_demo>> demonstra `FrozenJSON`; código-fonte no <<ex_explore0>>.

[[ex_explore0_demo]]
.`FrozenJSON`, do <<ex_explore0>>, permite ler atributos como `talk.name`, e invocar métodos como `++feed.keys()++` e `++feed.items()++`
====
[source, python]
----
...
<<lineitem_class_v2prop_class>>) seria um bom candidato a se tornar uma
