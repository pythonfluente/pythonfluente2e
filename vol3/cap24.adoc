...
O código para `record_factory` está no <<record_factory_ex>>.footnote:[Agradeço ao meu amigo J. S. O. Bueno por ter contribuído com esse exemplo.]
...
algumas desvantagens, discutidas na <<slots_sec>>.
...
No https://fpy.li/5[«Capítulo 5»] (vol.1), vimos como `typing.NamedTuple` e `@dataclass` usam a sintaxe da instrução `class` para especificar atributos para uma
...
Usar tipos embutidos para as anotações no <<checked_demo1_ex>> significa que os valores devem ser aceitáveis pelo construtor do tipo.
...
Veremos um uso criativo para ele na <<metahack_sec>>.
...
_Pássaros aquáticos e as ABCs_, na <<goose_typing_sec>>:
...
[quote, Brian Harvey e Matthew Wright, no prefácio de <em>Simply Scheme</em>]
____
