...
Mas, tudo pode mudar trocando `sleep()` por `is_prime()` (<<def_is_prime_ex>>).
...
Observe a <<one_possible_architecture_fig>>, um dos muitos diagramas de arquitetura do livro de Kleppmann.
...
<<app_server_fig>> inteira substituiria o
retângulo sólido _Application code_ (código da
aplicação) no topo da <<one_possible_architecture_fig>>.
